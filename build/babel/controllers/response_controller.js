...
Encompass.ResponseController = Ember.Controller.extend({
  ...
  greeting: function () {
    ...
    var promise = new Ember.RSVP.Promise(function (resolve, reject) {
      controller.get('model.submission').then(function (sub) {
        ...
        var firstname = brk === -1 ? controller.get('model.student') : controller.get('model.student').substr(0, brk);
  ...
  quote: function quote(string, opts) {
      ...
      controller.get('model.selections').then(function (selections) {
        if (selections) {
          selections.forEach(function (s) {
            ...
            controller.get('model.comments').filterBy('selection.id', s.get('id')).forEach(function (c) {
              var opts = {
                type: c.get('label'),
                usePrefix: true
              };
  ...
  _persistThen: function _persistThen(callback) {
  ...
  actions: {
    toggleProperty: function toggleProperty(p) {

...
Encompass.FoldersEditController = Ember.Controller.extend(Encompass.CurrentUserMixin, {
  ...
  */
  actions: {
    ...
    changeSubmission: function changeSubmission(submission) {
    ...
    changeSelection: function changeSelection(selection) {
      ...
      Ember.run(function () {
        ...
        if (window.opener) {
          ...
          controller.transitionToRoute('workspace.submission.selection', selection.get('workspace'), selection.get('submission'), selection);
        }
      });
    },

    removeSelection: function removeSelection(selection, folder) {
      //ENC-574 Because a selection may also be in a sub-folder, also make the folder a param 
...
//# sourceMappingURL=folders_edit_controller.js.map

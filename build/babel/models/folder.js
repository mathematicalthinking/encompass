...
Encompass.Folder = DS.Model.extend(Encompass.Auditable, {
  ...
  selections: function () {
    return this.get('taggings').filterBy('isTrashed', false).getEach('selection').filterBy('isTrashed', false);
  }.property('taggings.[]', 'taggedSelections.@each.isTrashed'),

  childSelections: function () {
    var selections = this.get('selections').toArray();
    this.get('children').filterBy('isTrashed', false).getEach('_selections').forEach(function (childSelections) {
  ...
  submissions: function () {
    return this.get('selections').filterBy('isTrashed', false).getEach('submission').uniq();
  }.property('selections.@each.isTrashed'),

  _submissions: function () {
    var submissions = this.get('submissions').toArray();

    this.get('children').filterBy('isTrashed', false).getEach('_submissions').forEach(function (childSubmissions) {
  ...
  hasSelection: function hasSelection(selectionId) {
    return this.get('taggings').isAny('selection.id', selectionId);
  },

  filteredChildren: function () {
    return this.get('children').filterBy('isTrashed', false).sortBy('weight', 'name');

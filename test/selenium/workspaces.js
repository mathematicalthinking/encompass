...
describe('Visiting Workspaces', function () {
  ...
  describe('Visiting ESI 2014 Wednesday Reflection', function () {
    it('should render workspace', async function () {
      ...
      await helpers.waitForAndClickElement(
        driver,
        `a[href="#/workspaces/${workspaceId}/work"]`
      );
      ...
      expect(url).to.equal(
        `${host}/workspaces/${workspaceId}/submissions/53e36522729e9ef59ba7f4de`
      );
    });
    ...
    it('should display submission navigation arrows, and revision links', async function () {
      ...
      expect(await helpers.findAndGetText(driver, 'ul.breadcrumbs')).to.contain(
        '1'
      );
    });
    ...
    it('should display a select box for students', async function () {
      expect(
        await helpers.isElementVisible(driver, css.workspace.studentsSelect)
      ).to.eql(true);

      return helpers.waitForSelectizeSingleText(
        driver,
        '#student-select',
        'Andrew S.'
      );
    ...
    it('should show the short answer', async function () {
      ...
      expect(await helpers.findAndGetText(driver, selector)).to.contain(
        'The most useful part of today was having Max'
      );
    });
    ...
    it('should have selecting enabled by default', async function () {
      ...
      try {
        let checkbox = await driver.wait(
          until.elementLocated(By.css('label.makingSelection>input')),
          3000
        );
        if (checkbox) {
  ...
  describe('clicking on the student dropdown', function () {
    ...
    before(async function () {
      try {
        selectBox = await driver.findElement(
          By.css(css.workspace.studentsSelect)
        );
        await selectBox.click();
        studentList = await helpers.waitForSelector(
          driver,
          css.workspace.dropdownContent
        );
      } catch (err) {
        console.log(err);
      }
  ...
  describe('Visiting a Selection in ESI 2014', function () {
    before(async function () {
      ...
      await helpers.waitForAndClickElement(
        driver,
        `a[href="#/workspaces/${workspaceId}/submissions/${submissionId}/selections/${selectionId}"]`
      );
    ...
    it('should display a bunch of submissions', async function () {
      ...
      expect(currentUrl).to.match(
        /workspaces\/.*\/submissions\/.*\/selections\//
      );
    ...
    xit('should display a bunch of comments', async function () {
      //this comment isn't connected to the proper submission
      let comments;
      let commentsText;
      await driver.sleep(5000);
      try {
        comments = await driver.findElements(
          By.css('#al_feedback_display>ul>li')
        );
        commentsText = await Promise.all(
          comments.map((el) => {
            return el.getText();
          })
        );
      ...
      expect(commentsText[0]).to.contain(
        'I spoke with Michael about the balance'
      );
    });
    ...
    it('should display a bunch of folders', async function () {
      ...
      let folderNames;
      try {
        folders = await driver.wait(
          until.elementsLocated(By.css('#al_folders>li')),
          3000
        );
  ...
  describe('Visiting a Folder from ESI 2014', function () {
    ...
    describe('clicking the folder icon', function () {
      it('should display sub-folders (if any)', async function () {
        ...
        expect(await helpers.isElementVisible(driver, 'li>ul.subfolders')).to.be
          .false;

        try {
          let links = await driver.wait(
            until.elementsLocated(By.css('span.toggle-icon.branch')),
            3000
          );
          if (links) {
            await links[0].click();
            subFolderList = await driver.wait(
              until.elementLocated(By.css('ul.subfolders'))
            );
            if (subFolderList) {
              subFolders = await subFolderList.findElements(
                By.css('.folderItem')
              );
            }
          }

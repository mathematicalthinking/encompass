...
describe('Mentoring Interactions', function () {
  ...
  describe('Visting Responses List', function () {
    ...
    it('should display solver tab and display count', async function () {
      ...
      let submitterTabs = await helpers.getWebElements(
        driver,
        css.responsesList.submitterTab
      );
    ...
    it('should not display mentoring tab', async function () {
      expect(
        await helpers.isElementVisible(driver, css.responsesList.mentoringTab)
      ).to.eql(false);
    });
    ...
    it('should have one response thread in list', async function () {
      let responseThreads = await helpers.waitForNElements(
        driver,
  ...
  describe('Viewing response in paneled view', function () {
    ...
    it('should display submission view', function () {
      return helpers
        .waitForElementToHaveText(
          driver,
          submitterCss.studentIndicator,
          feedbackReceiver.username
        )
        .catch((err) => {
          throw err;
        });
    ...
    it('should display mentor reply view', function () {
      return Promise.all([
        helpers.waitForElementToHaveText(
          driver,
    ...
    it('should show revise button', function () {
      ...
      return helpers
        .waitForAndClickElement(driver, submitterCss.reviseBtn)
        .then(() => {
    ...
    describe('Submitting revision from response page', function () {
      ...
      it('should not submit revision if no changes are made', async function () {
        ...
        expect(await helpers.isElementVisible(driver, submitBtnClass)).to.eql(
          true
        );
      });
      ...
      it('should successfully create revision', async function () {
        ...
        await helpers.findInputAndType(
          driver,
          answerNewCss.inputs.explanation,
          newExplanation
        );
        ...
        let revItems = await helpers.waitForNElements(
          driver,
          submitterCss.revIndexItem,
          2
        );

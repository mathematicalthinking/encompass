...
describe('Workspace info / settings interactions', function () {
  ...
  describe('Navigating to workspace info page', function () {
    before(async function () {
      await helpers.navigateAndWait(driver, wsInfoUrl, {
    ...
    it('should show workspace settings container', async function () {
      expect(
        await helpers.isElementVisible(driver, selectors.settings.container)
      ).to.eql(true);
    });

    it('should show workspace collabs container', async function () {
      expect(
        await helpers.isElementVisible(driver, selectors.collabs.container)
      ).to.eql(true);
    });

    it('should show workspace stats container', async function () {
      expect(
        await helpers.isElementVisible(driver, selectors.stats.container)
      ).to.eql(true);
    });
  });
  ...
  describe('Changing workspace name', function () {
    ...
    it('should display success toast message', async function () {
      expect(
        await helpers.isTextInDom(driver, selectors.settings.updateSuccessText)
      ).to.eql(true);
    });

    it('should display new name', async function () {
      expect(
        await helpers.findAndGetText(driver, selectors.settings.nameText)
      ).to.eql(newName);
    });

    it('should display new name after page refresh', async function () {
      await helpers.navigateAndWait(driver, wsInfoUrl, {
        selector: selectors.settings.container,
      });
      expect(
        await helpers.findAndGetText(driver, selectors.settings.nameText)
      ).to.eql(newName);
    ...
    describe('Toggling from yes to no', function () {
      before(async function () {
        ...
        await helpers.selectOption(
          driver,
          selectors.settings.autoUpdateSelect,
          'No',
          true
        );
      ...
      it('should display success toast message', async function () {
        expect(
          await helpers.isTextInDom(
            driver,
            selectors.settings.updateSuccessText
          )
        ).to.eql(true);
      });
      ...
      it('should display No for automatic updates', async function () {
        expect(
          await helpers.findAndGetText(
            driver,
            selectors.settings.autoUpdateText
          )
        ).to.eql('No');
      });
  ...
  describe('Setting Linked Assignment', function () {
    before(async function () {
      await openEditMenu();
      let input = await helpers.getWebElements(
        driver,
        selectors.settings.linkedAssnInput
      );
      ...
      expect(
        await helpers.findAndGetText(driver, selectors.settings.linkedAssnText)
      ).to.contain(assignmentToLink.name);
      await saveSettings();
    });
  ...
  let { addBtn, usernameInput, usernameText, saveCollab, collabItems } =
    css.wsInfo.collabs;
  ...
  describe('Adding / removing collaborators', function () {
    before(async function () {
      ...
      expect(await helpers.findAndGetText(driver, usernameText)).to.eql(
        collabToAdd.username
      );
    ...
    it('should successfully add the user as collab', async function () {
      ...
      expect(await helpers.findAndGetText(driver, 'div.collab-name')).to.eql(
        collabToAdd.username
      );
    });
    ...
    it('should display new collab after page refresh', async function () {
      ...
      let items = await helpers.getWebElements(driver, collabItems);
      expect(items).to.have.lengthOf(1);
      expect(await helpers.isTextInDom(driver, collabToAdd.username)).to.eql(
        true
      );
    });
  });

...
const {
  ...
  assignment,
  originalLinkedWs,
} = require('./fixtures/linked_workspaces');
  ...
  describe('Linking new workspace to assignment', function () {
    before(async function () {
      await helpers.navigateAndWait(driver, newWsInfoUrl, {
        selector: wsInfoSelectors.container,
      });
    });
    ...
    describe('Setting Linked Assignment', function () {
      before(async function () {
        ...
        let input = await helpers.getWebElements(
          driver,
          wsInfoSelectors.settings.linkedAssnInput
        );
        ...
        expect(
          await helpers.findAndGetText(
            driver,
            wsInfoSelectors.settings.linkedAssnText
          )
      ...
      it('should display success toast message', async function () {
        expect(
          await helpers.isTextInDom(
            driver,
            wsInfoSelectors.settings.updateSuccessText
          )
        ).to.eql(true);
      });
  ...
  describe('Visiting assignment page', function () {
    before(async function () {
      return await helpers
        .navigateAndWait(driver, assignmentInfoUrl, {
          selector: css.assignmentsTeacher.editAssignment,
        })
        .catch((err) => {
          throw err;
        });
    ...
    describe('Creating response in newly linked workspace', function () {
      it('should create response successfully', async function () {
        await helpers.navigateAndWait(driver, newWsUrlFirstSub, {
          selector: css.workspace.container,
        });
  ...
  describe('Revising from responses page', function () {
    ...
    describe('For original linked workspace', function () {
      ...
      describe('Submitting revision from response page', function () {
        ...
        it('linked workspace should have been updated', async function () {
          ...
          ).to.eql((workspace.originalSubmissionCount + 1).toString());
        });
      });
    });
    ...
    describe('For newly linked workspace', function () {
      ...
      describe('Visiting assignment page', function () {
        before(async function () {
          await helpers.login(driver, host, owner);
          await helpers.navigateAndWait(driver, assignmentInfoUrl, {
            selector: css.assignmentsTeacher.editAssignment,
          });
        });
        ...
        it('should have automatically updated assignment revision counts', async function () {
          ...
          let didAppear = await helpers.waitForTextInDom(
            driver,
            `for a total of ${updatedCount} submissions`
          );

          ...
          describe('Visiting problems main page', function () {
            before(async function () {
              ...
              if (!isStudent) {
                await helpers.waitForSelector(
                  driver,
                  css.problemPageSelectors.problemContainer
                );
              }
            });
            ...
            it('should display side list of filter options', async function () {
              if (!isStudent) {
                expect(
                  await helpers.waitForSelector(
                    driver,
                    css.problemPageSelectors.sideFilterOptions
                  )
                );
              }
              let optionsList = css.problemFilterList.primaryFilters;
              let filterOptions = helpers.createFilterList(
                isStudent,
                isAdmin,
                optionsList,
                true
              ...
              expect(await helpers.checkSelectorsExist(driver, filterSelectors))
                .to.be.true;
            });
            ...
            it('should display category filter options', async function () {
              if (!isStudent) {
                ...
                let filterSelectors = helpers.createSelectors(
                  css.problemFilterList.categoryFilters
                );
                expect(
                  await helpers.checkSelectorsExist(driver, filterSelectors)
                ).to.be.true;
                await helpers.findAndClickElement(driver, '.category-header');
              }
            });
            ...
            if (!isStudent) {
              describe('Clicking on My Org filter option', function () {
                ...
                it('should update problem list when unchecking created by members', async function () {
                  if (!isStudent) {
                    await helpers.waitForAndClickElement(
                      driver,
                      'li.fromOrg label.checkbox-label'
                    );
                  ...
                  if (!isStudent) {
                    expect(
                      await helpers.isElementVisible(
                        driver,
                        '#problem-list-ul li:first-child .item-section.name span:nth-child(2)'
                      )
                    ).to.be.true;
                  }
                });
                ...
                it('should update problem list when unchecking recommended', async function () {
                  if (!isStudent) {
                    await helpers.waitForAndClickElement(
                      driver,
                      'li.recommended label.checkbox-label'
                    );
                    await helpers.waitForSelector(driver, css.resultsMessage);
                    await helpers.waitForTextInDom(driver, css.noResultsMsg);
                    expect(
                      await helpers.findAndGetText(driver, css.resultsMessage)
                    ).to.contain(css.noResultsMsg);
                  }
                });
              ...
              describe('Clicking on Category filter menu', function () {
                ...
                it('should show problems when adding category with problems to list', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      '[data-value="5bb650e1fefbf3cf9e88f673"]'
                    );
                    await helpers.waitForTextInDom(driver, resultsMsg);
                    expect(
                      await helpers.findAndGetText(driver, css.resultsMessage)
                ...
                it('should show more problems when adding category with problems to list', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      '[data-value="5bb650e1fefbf3cf9e88f675"]'
                    );
                    await helpers.waitForTextInDom(driver, resultsMsg);
                    expect(
                      await helpers.findAndGetText(driver, css.resultsMessage)
                ...
                it('should show less problems when removing a category', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      'ul.selected-cat-list li:first-child i'
                    );
                    await driver.sleep(500);
                    await helpers.waitForTextInDom(driver, resultsMsg);
                    expect(
                ...
                it('should open up category menu modal', async function () {
                  if (!isStudent) {
                    ...
                    expect(
                      await helpers.findAndGetText(driver, '.modal-content h4')
                    ).to.contain('Select a Category');
                  }
                });
                ...
                it('should add category when clicked', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      'button[id="CCSS.Math.Content.K.G.B.6"]'
                    );
                    await driver.sleep(300);
              ...
              xdescribe('Testing search bar', function () {
                ...
                it('should show clear button with empty search box and query applied', async function () {
                  if (!isStudent) {
                    ...
                    expect(await helpers.isElementVisible(driver, 'svg.clear'))
                      .to.be.true;
                    await helpers.findAndClickElement(driver, 'svg.clear');
                  }
                });
                ...
                it('should throw and error when search field is empty', async function () {
                  if (!isStudent) {
                    let errorMessage =
                      'Query is too short (minimum is 1 characters)';
                    ...
                    expect(
                      await helpers.findAndGetText(driver, '.error-text')
                ...
                it('should throw and error when search is too long', async function () {
                  if (!isStudent) {
                    let errorMessage =
                      'Query is too long (maximum is 500 characters)';
                    await helpers.findInputAndType(
                      driver,
                      '.search-field',
                      css.longString,
              ...
              describe('Testing layout and refresh', function () {
                ...
                it('should show card view when clicking on grid', async function () {
                  if (!isStudent) {
                    ...
                    expect(await helpers.isElementVisible(driver, '.grid-view'))
                      .to.be.true;
                  }
                });
                ...
                it('should show list view when clicking on list view', async function () {
                  if (!isStudent) {
                    ...
                    expect(
                      await helpers.isElementVisible(driver, '#layout-view')
                    ).to.be.true;
                  }
                });
              });
              ...
              describe('Testing sortbar functionality', function () {
                ...
                it('should have sortbar with many options', async function () {
                  if (!isStudent) {
                    let selectors = [
                      '.sort-bar-item.privacy',
                      '.sort-bar-item.name',
                      '.sort-bar-item.date',
                      '.sort-bar-item.status',
                    ];
                    expect(await helpers.checkSelectorsExist(driver, selectors))
                      .to.be.true;
                  }
                });
                ...
                it('should be sorted alphabetically', async function () {
                  if (!isStudent) {
                    ...
                    ).to.contain('Alphabetical Problem');
                  }
                });
                ...
                it('should be filtered by private', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      '#privacy-menu ul li .radio-filter .radio-label input[value="M"]'
                    );
                    await driver.sleep(500);
                    await helpers.waitForTextInDom(driver, resultsMsg);
                    expect(
                ...
                it('should filter out approved problems', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      '.hover-menu ul li label input[value="approved"]'
                    );
                    await driver.sleep(500);
                    await helpers.waitForTextInDom(driver, resultsMsg);
                    expect(
                ...
                it('should filter out pending problems', async function () {
                  if (!isStudent) {
                    ...
                    await helpers.findAndClickElement(
                      driver,
                      '.hover-menu ul li label input[value="pending"]'
                    );
                    await driver.sleep(500);
                    await helpers.waitForTextInDom(driver, resultsMsg);
                    expect(
                ...
                it('should filter out flagged and only show approved problems', async function () {
                  if (!isStudent) {
                    ...
                    if (!isTeacher) {
                      ...
                      await helpers.findAndClickElement(
                        driver,
                        '.hover-menu ul li label input[value="flagged"]'
                      );
                    }
                    await helpers.findAndClickElement(
                      driver,
              ...
              describe('Testing problem list item functionality', function () {
                ...
                it('clicking on problem description should show problem info', async function () {
                  if (!isStudent) {
                    await helpers.findAndClickElement(
                      driver,
                      '#problem-list-ul li:first-child .item-section.description span:first-child'
                    );
                    await driver.sleep(500);
                    let selectors = ['.info-header', '.side-info-menu'];
                    expect(await helpers.checkSelectorsExist(driver, selectors))
                ...
                describe('Testing action button functionality', function () {
                  ...
                  if (isTeacher) {
                    it('problem action button should be assign', async function () {
                      let selector =
                        '#problem-list-ul li:first-child .item-section.action button.primary-button';
                      let buttonText = 'Assign';
                      expect(
                        await helpers.findAndGetText(driver, selector)
                      ).to.contain(buttonText);
                    });
                    ...
                    it('clicking assign button should open assign view', async function () {
                      ...
                      await driver.sleep(500);
                      await helpers.waitForTextInDom(
                        driver,
                  ...
                  if (isPdadmin) {
                    ...
                    it('flagged problem action button should be copy', async function () {
                      let selector =
                        '#problem-list-ul li:nth-child(2) .item-section.action button.primary-button';
                      let buttonText = 'Copy';
                      expect(
                        await helpers.findAndGetText(driver, selector)
                      ).to.contain(buttonText);
                    ...
                    it('clicking copy button should copy flagged problem to mine', async function () {
                      ...
                      ).to.contain('Copy of Flagged Problem');
                    });
                  }
                  ...
                  if (isAdmin) {
                    ...
                    it('clicking flag button should open flag modal and flag problem', async function () {
                      ...
                      expect(
                        await helpers.findAndGetText(
                          driver,
                          '#problem-list-ul li:first-child .item-section.status span.status-text',
                          true
                        )
                      ).to.contain('flagged');
                    });
                ...
                describe('Testing more button menu listing', function () {
                  ...
                  if (!isAdmin) {
                    ...
                    xit('problem more button should only show report', async function () {
                      let selectors = [
                        ...
                        '#container > div.item-section.more > span > ul > li > label > span',
                      ];
                      ...
                      expect(
                        await helpers.isElementVisible(driver, reportIconSel)
                      ).to.be.true;
                      ...
                      expect(
                        await helpers.isElementVisible(driver, editIconSel)
                      ).to.be.false;
                      ...
                      expect(
                        await helpers.isElementVisible(driver, trashIconSel)
                      ).to.be.false;
                    });
                    if (isPdadmin) {
                  ...
                  if (isAdmin) {
                    ...
                    it('problem more for mine pending should show 2 options', async function () {
                      ...
                      await helpers.findAndClickElement(
                        driver,
                        '#problem-list-ul li:nth-child(2) .item-section.more'
                      );
                      await driver.sleep(500);
                      expect(
                        await helpers.getWebElements(
                          driver,
                          '.item-section.more span.click-menu ul>li'
                        )
              ...
              describe('Testing problem card view functionality', function () {
                ...
                it('clicking on problem title should show problem info', async function () {
                  if (!isStudent) {
                    await helpers.findAndClickElement(
                      driver,
                      '#problem-list-ul li:first-child .item-card.name a'
                    );
                    await driver.sleep(500);
                    let selectors = ['.info-header', '.side-info-menu'];
                    expect(await helpers.checkSelectorsExist(driver, selectors))
                ...
                it('clicking on problem description should show problem info', async function () {
                  if (!isStudent) {
                    await helpers.findAndClickElement(
                      driver,
                      '#problem-list-ul li:first-child .item-card.description a'
                    );
                    await driver.sleep(500);
                    let selectors = ['.info-header', '.side-info-menu'];
                    expect(await helpers.checkSelectorsExist(driver, selectors))
            ...
            if (isAdmin) {
              describe('Clicking on Trashed problems', function () {
                ...
                it('should update problem list and display message', function () {
                  ...
                  return helpers
                    .findAndClickElement(driver, '#toggle-trashed')
                    .then(async () => {
                ...
                it('should restore a trashed problem', async function () {
                  await helpers.saveScreenshot(driver);
                  let list = await helpers.waitForSelector(
                    driver,
                    '#problem-list-ul'
                  );
                  let firstItem = await list.findElement({ xpath: './li' });

                  let btn = await firstItem.findElement({
                    css: '.primary-button',
                  });
                  await btn.click();
                  await helpers.saveScreenshot(driver);
                ...
                it('should continue displaying trashed problems until unchecked', async function () {
                  ...
                  await helpers.findAndClickElement(
                    driver,
                    '.more-filter-list .subfilter input#toggle-trashed'
                  );
              ...
              describe('Clicking on All problems filter', function () {
                ...
                describe('Searching by creator', function () {
                  before(async function () {
                    await helpers.findAndClickElement(
                      driver,
                      '#admin-filter-select-selectized'
                    );
                    await helpers.findAndClickElement(
                      driver,
                      '[data-value="creator"]'
                    ...
                    await helpers.findInputAndType(
                      driver,
                      '#all-user-filter-selectized',
                      'morty'
                    );
                    await helpers.waitForSelector(
                      driver,
                      '[data-value="5b245841ac75842be3189526"]'
                    );
                    await helpers.findAndClickElement(
                      driver,
                      '[data-value="5b245841ac75842be3189526"]'
                ...
                describe('Searching by PoWs', function () {
                  ...
                  it('should uncheck public then update the list and display message', async function () {
                    await helpers.findAndClickElement(
                      driver,
                      'li.shared label.checkbox-label'
                    );
                    await helpers.waitForSelector(driver, css.resultsMessage);
                  ...
                  it('should uncheck private then show no problems', async function () {
                    await helpers.findAndClickElement(
                      driver,
                      'li.unshared label.checkbox-label'
                    );
                    await helpers.waitForSelector(driver, css.resultsMessage);
                    await helpers.waitForTextInDom(driver, css.noResultsMsg);
                    expect(
            ...
            describe('Testing more button functions', function () {
              ...
              if (isTeacher) {
                ...
                it('delete button in more menu should show delete modal and remove problem', async function () {
                  ...
                  await helpers.findAndClickElement(
                    driver,
                    '.item-section.more span.click-menu ul li label i.fa-trash'
                  );
                  await driver.sleep(500);
                  await helpers.findAndClickElement(
                    driver,
                  ...
                  ).to.contain("Summer's Org Problem");
                });
                it('report button in more menu should report modal and mark problem as flagged', async function () {
                  ...
                  await helpers.findAndClickElement(
                    driver,
                    '.item-section.more span.click-menu ul li label i.fa-exclamation-circle'
                  );
                  await driver.sleep(500);
                  await helpers.findAndClickElement(
                    driver,
              ...
              if (isAdmin) {
                ...
                it('pending button in more menu should show pending modal and mark as pending', async function () {
                  ...
                  await helpers.findAndClickElement(
                    driver,
                    '.item-section.more span.click-menu ul li label i.fa-clock'
                  );
                  await driver.sleep(500);
                  await helpers.findAndClickElement(
                    driver,
                ...
                it('assign button in more menu should show problem assign view', async function () {
                  ...
                  await helpers.findAndClickElement(
                    driver,
                    '.item-section.more span.click-menu ul li label i.fa-list-ul'
                  );
                  await driver.sleep(500);
                  await helpers.waitForTextInDom(
                    driver,
                  ...
                  expect(
                    await helpers.findAndGetText(driver, '#assignmentnewheader')
                  ).to.contain('Create New Assignment');
                  await helpers.findAndClickElement(
                    driver,
                    'button.cancel-button'

    ...
    function _runTests(user) {
      ...
      describe(`As ${testDescriptionTitle}`, function () {
        ...
        describe('Visiting sections page', function () {
          before(async function () {
            await helpers.navigateAndWait(driver, `${helpers.host}/sections`, {
              selector: 'ul.your-sections',
            });
          });
          it("should display list of user's own sections", async function () {
            expect(
              await helpers.getWebElements(driver, 'ul.your-sections a')
            ).to.have.lengthOf(sections.own.count);
          });
          if (!isStudent) {
            it('should display list of sections the user belongs to', async function () {
              expect(
                await helpers.getWebElements(driver, 'ul.collab-sections a')
              ).to.have.lengthOf(sections.collab.count);
            });
            if (accountType === 'A') {
              it('should display list of all sections', async function () {
                expect(
                  await helpers.getWebElements(driver, 'ul.all-sections a')
                ).to.have.lengthOf(sections.all.count);
              });
            } else if (accountType === 'P') {
              it("should display list sections for user's org", async function () {
                expect(
                  await helpers.getWebElements(driver, 'ul.org-sections a')
                ).to.have.lengthOf(sections.org.count);
              });
            }
          }
        });
        ...
        describe(`Visiting ${sectionDetails.name}`, function () {
          ...
          it('should display the section details', async function () {
            ...
            expect(
              await helpers.findAndGetText(driver, css.sectionInfo.details.name)
            ).to.eql(sectionDetails.name);
          });
          if (!isStudent) {
            describe('adding a student to class', function () {
              before(async function () {
                await helpers.waitForSelector(
                  driver,
                  css.sectionInfo.editButtons.students
                );
              });
              ...
              let usernameLinkSelector = `${hash[username][0]}`;
              it('clicking on edit students should bring up menus', async function () {
                ...
                await helpers.selectSingleSelectizeItem(
                  ...
                  hash[username][0],
                  hash[username][1],
                  { willInputClearOnSelect: true, toastText: 'Student Added' }
                );

                expect(
                  await helpers.isTextInDom(driver, usernameLinkSelector)
                ).to.eql(true);
              });
              ...
              if (accountType === 'T' && input === 'teacher') {
                ...
                it(`teacher field should be fixed as current user`, async function () {
                  expect(
                    await helpers.findAndGetText(
                      driver,
                      css.newSection.fixedInputs.teacher
                    )
                  ).to.eql(username);
                });
                ...
                if (accountType === 'A') {
                  ...
                  it(`organization field should be fixed as user's org`, async function () {
                    expect(
                      await helpers.findAndGetText(
                        css.newSection.fixedInputs.organization
                      )
                    ).to.eql(organization);
                  });
                }
                ...
                it(`${input} field should be visible`, async function () {
                  expect(await helpers.isElementVisible(driver, inputs[input]))
                    .to.be.true;
                });
              }
            }
            ...
            describe('Creating section', function () {
              ...
              describe('submitting empty form', function () {
                it('should display error message(s)', async function () {
                  ...
                  await helpers.waitForSelector(
                    driver,
                    css.general.errorMessage
                  );
                  expect(await helpers.getCurrentUrl(driver)).to.match(
                    /sections\/new/
                  );
                  expect(await helpers.isTextInDom(driver, nameError)).to.be
                    .true;
                });
              });

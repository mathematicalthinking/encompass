...
describe('Users', function () {
  ...
  function clearAndTypeInput(driver, inputType, text) {
    return clearInput(driver, inputType).then(() => {
  ...
  describe('Logged in as an admin user', function () {
    ...
    function validateUsersPage() {
      it('should show/hide various editable fields', async function () {
        ...
        expect(await helpers.isTextInDom(driver, helpers.admin.username)).to.be
          .true;
    ...
    function validateNewUserPage() {
      it('should display the page title and form', async function () {
        ...
        expect(await helpers.isElementVisible(driver, 'article.user')).to.be
          .true;
      });

      it('should show certain fields', async function () {
        expect(await helpers.isElementVisible(driver, 'input.user-username')).to
          .be.true;
        expect(await helpers.isElementVisible(driver, 'input.user-password')).to
          .be.true;
        expect(await helpers.isElementVisible(driver, 'input.user-first-name'))
          .to.be.true;
        expect(await helpers.isElementVisible(driver, 'input.user-last-name'))
          .to.be.true;
        expect(await helpers.isElementVisible(driver, 'input.user-email')).to.be
          .true;
        expect(await helpers.isElementVisible(driver, 'input.typeahead')).to.be
          .true;
      ...
      it('should not let you submit form with missing fields', async function () {
        ...
        expect(await helpers.findAndGetText(driver, '.error-box')).to.contain(
      ...
      it('should let you create a new unauthorized pdadmin', async function () {
        ...
        expect(
          await helpers.findAndGetText(driver, 'ul.waiting-auth>li:first-child')
        ).to.contain('muzzy');
      });
      ...
      it('should let you create a new authorized teacher', async function () {
        ...
        expect(
          await helpers.findAndGetText(
            driver,
            'ul.teacher-users>li:first-child'
          )
        ).to.contain('msmith');
      });
      ...
      it('should let you create a new student without an email', async function () {
        ...
        expect(
          await helpers.findAndGetText(
            driver,
            'ul.student-users>li:first-child'
          )
        ).to.contain('newstudent');
      });
    }
    ...
    function changeAuth() {
      it('should authorize a new user', async function () {
        ...
        expect(
          await helpers.findAndGetText(driver, 'ul.pd-users>li:first-child')
        ).to.contain('muzzy');
      });
    }
    ...
    function changeAccountType() {
      it('should change a teacher to an admin', async function () {
        ...
        expect(
          await helpers.findAndGetText(driver, 'ul.admin-users>li:first-child')
        ).to.contain('nope');
      });
    }
    ...
    function confirmEmail() {
      it('should manually confirm email', async function () {
        ...
        await helpers.waitForSelector(driver, '#user-info');
        expect(
          await helpers.findAndGetText(driver, 'td.is-email-confirm')
        ).to.contain('true');
      });
    }
    ...
    describe('Visiting the users list home page', function () {
      before(async function () {
        await helpers.navigateAndWait(driver, `${host}/users`, {
          selector: '#user-home',
        });
      });
      ...
      it('should display a list with your account', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.your-account>li')
        ).to.have.lengthOf.at.least(1);
        expect(
          await helpers.findAndGetText(driver, 'ul.your-account>li:first-child')
        ).to.contain('rick');
      });

      it('should have a list of users waiting for authorization', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.waiting-auth>li')
      ...
      it('should have a list of admins', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.admin-users>li')
        ).to.have.lengthOf.at.least(6);
        expect(
          await helpers.findAndGetText(driver, 'ul.admin-users>li:first-child')
        ).to.contain('superuser');
      });

      it('should have a list of pd admins', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.pd-users>li')
        ).to.have.lengthOf.at.least(1);
        expect(
          await helpers.findAndGetText(driver, 'ul.pd-users>li:first-child')
        ).to.contain('mentorpd');
      ...
      it('should have a list of teachers', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.teacher-users>li')
        ).to.have.lengthOf.at.least(11);
        expect(
          await helpers.findAndGetText(
            driver,
            'ul.teacher-users>li:first-child'
          )
        ).to.contain('jl_picard');
      });

      it('should have a list of students', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.student-users>li')
        ).to.have.lengthOf(12);
      });
  ...
  describe('Logged in as a pd admin user', function () {
    ...
    function validateUsersPage() {
      it('should show/hide various editable fields', async function () {
        ...
        expect(await helpers.isTextInDom(driver, helpers.pdAdmin.username)).to
          .be.true;
  ...
  describe('Logged in as a teacher', function () {
    ...
    function validateNewUserPage() {
      let signupMessages = messages.signup;
      it('should display the page title and form', async function () {
        expect(await helpers.isTextInDom(driver, 'Create a New User')).to.be
          .true;
        expect(await helpers.isElementVisible(driver, 'article.user')).to.be
          .true;
      });
      ...
      it('should let you create a new student', async function () {
        ...
        await helpers.waitForSelector(driver, '#user-info');
        expect(
          await helpers.findAndGetText(driver, 'ul.your-users>li:first-child')
        ).to.contain('mystudent');
      });
    }
    ...
    describe('Visiting the users list home page', function () {
      ...
      it('should have a list of users you have created', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.your-users>li')
        ).to.have.lengthOf.at.least(1);
      });

      it('should have a list of users in your org', async function () {
        expect(
          await helpers.getWebElements(driver, 'ul.org-users>li')
        ).to.have.lengthOf.at.least(8);
        expect(
          await helpers.findAndGetText(driver, 'ul.org-users>li:first-child')
        ).to.contain('bunny');
      });

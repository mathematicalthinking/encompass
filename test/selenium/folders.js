  ...
  describe('Visiting a ESI 2014 Wednesday Reflection', function () {
    before(async function () {
      ...
      await helpers.navigateAndWait(
        driver,
        `${host}/workspaces/53e36522b48b12793f000d3b/folders/53e36cdbb48b12793f000d43`,
        options
      );
    });

    it('should display the folder name', async function () {
      expect(await helpers.findAndGetText(driver, 'div#menubar>h1')).to.eql(
        'Improve'
      );
    });
    ...
    it('should display view controls', async function () {
      ...
      expect(await helpers.isElementVisible(driver, '#subcontrols')).to.eql(
        true
      );
      expect(
        await helpers.getWebElements(driver, 'input[name="browser2"]')
      ).to.have.lengthOf(2);
      expect(
        await helpers.isElementVisible(driver, 'label#showEvidence>input')
      ).to.eql(true);
      // expect(await helpers.isElementVisible(driver, 'label#showSubmComments>input')).to.eql(true);
      expect(
        await helpers.isElementVisible(driver, 'label#showSubFolders>input')
      ).to.eql(true);
    ...
    it('should have default view options selected', async function () {
      ...
      try {
        let showSubFolders = await helpers.getWebElements(
          driver,
          'label#showSubFolders>input'
        );
        ...
        let showSubmFolders = await helpers.getWebElements(
          driver,
          'label#showSubmFolders>input'
        );
    ...
    it('should display a table of submission/selection data', async function () {
      ...
      try {
        isTableVisible = await helpers.isElementVisible(
          driver,
          'table#folder_contents'
        );
        let tableRows = await helpers.getWebElements(
          driver,
          'table#folder_contents>tbody>tr'
        );

    ...
    function _runTests(user) {
      ...
      describe(`As ${testDescriptionTitle}`, function () {
        ...
        describe('Visiting assignments page', function () {
          before(async function () {
            await helpers.navigateAndWait(driver, assignmentLink, {
              selector: 'div',
            });
            await helpers.waitForSelector(
              driver,
              css.assignmentsStudent.ownList
            );
          });
          it('should display list of assignments', async function () {
            ...
            expect(
              await helpers.getWebElements(
                driver,
                `${css.assignmentsStudent.ownList} a`
              )
            ).to.have.lengthOf(assignments.own.count);
          });
        });
        ...
        describe(`Visting ${assignmentDetails.name}`, function () {
          before(async function () {
            ...
            await helpers.waitForSelector(
              driver,
              css.assignmentsStudent.infoPage.container
            );
          });

          it('should display assignment details', async function () {
            let results = await Promise.all(
              _.map(assignmentDetails, (val, key) => {
                if (key === '_id') {
                  return true;
                }
                driver.sleep(500);
        ...
        describe(`Submitting response to assignment`, function () {
          ...
          if (submitDetails.isRevision) {
            ...
            it('brief summary should be prefilled', async function () {
              expect(
                await helpers.getWebElementValue(
                  driver,
                  newAnswerSelectors.inputs.briefSummary
                )
              ).to.eql(submitDetails.oldAnswer.briefSummary);
            });
            it('explanation should be prefilled', async function () {
              expect(
                await helpers.findAndGetText(
                  driver,
                  newAnswerSelectors.inputs.explanation
                )
              ).to.eql(submitDetails.oldAnswer.explanation);
            });
            ...
            it('contributors should list all contributors from previous answer', async function () {
              let listItems = await helpers.getWebElements(
                driver,
                `${newAnswerSelectors.inputs.studentList} li`
              );
              expect(listItems).to.have.lengthOf(
                submitDetails.oldAnswer.contributors.length
              );
            ...
            xit('should display errors if empty form is submitted', async function () {
              ...
              await helpers.findAndClickElement(
                driver,
                newAnswerSelectors.createBtn
              );
              let errors = await helpers.waitForNElements(
                driver,
                'div.error-box',
                numExpectedErrors
              );
              ...
              expect(await helpers.isTextInDom(driver, `can't be blank`)).to.be
                .true;
            });
            ...
            it('should display error if explanation is omitted', async function () {
              await helpers.findInputAndType(
                driver,
                newAnswerSelectors.inputs.briefSummary,
                submitDetails.newAnswer.briefSummary
              );
              ...
              let errors = await helpers.getWebElements(
                driver,
                'div.error-box'
              );
              ...
              expect(
                await helpers.isTextInDom(driver, `Explanation can't be blank`)
              ).to.be.true;
            });
            ...
            it('should display error if brief summary is omitted', async function () {
              ...
              await helpers.findInputAndType(
                driver,
                newAnswerSelectors.inputs.explanation,
                submitDetails.newAnswer.explanation
              );
          ...
          it('should succesfully create answer', async function () {
            ...
            await helpers.waitForSelector(
              driver,
              css.assignmentsStudent.infoPage.pastSubsHeader
            );
            await driver.wait(async function () {
              let pastAnswers = await helpers.getWebElements(
                driver,
                'ol.submission-list li'
              );
            ...
            if (submitDetails.isRevision) {
              await driver.get(
                `${host}/#/workspaces/${submitDetails.newAnswer.workspacesToUpdate[0]}/work`
              );
              ...
              expect(
                await helpers.findAndGetText(driver, 'span.submission_count')
              ).to.eql('2');
          ...
          describe('Viewing most recent submission', function () {
            ...
            before(async function () {
              let items = await helpers.getWebElements(
                driver,
                `${css.assignmentsStudent.infoPage.subList} li`
              );
              await items[0].click();
              await helpers.waitForSelector(
                driver,
                css.assignmentsStudent.answerInfo.container
              );
            });
            ...
            it('should display correct brief summary', async function () {
              expect(
                await helpers.findAndGetText(
                  driver,
                  css.assignmentsStudent.answerInfo.briefSummary
                )
              ).to.eql(submitDetails.newAnswer.briefSummary);
            });
            ...
            it('should display correct explanation', async function () {
              expect(
                await helpers.findAndGetText(
                  driver,
                  css.assignmentsStudent.answerInfo.explanation
                )
              ).to.contain(submitDetails.newAnswer.explanation);
            });

            it('should display correct contributors', async function () {
              let items = await helpers.getWebElements(
                driver,
                `${css.assignmentsStudent.answerInfo.studentList} li`
              );
              expect(items).to.have.lengthOf(
                submitDetails.newAnswer.contributors.length
              );
            });

...
describe('Resetting Password', function () {
  ...
  describe('Invalid token', function () {
    before(async function () {
      await helpers.navigateAndWait(driver, invalidResetLink, {
        selector: css.resetPassword.invalidToken,
      });
    });

    it('should not display reset form', async function () {
      expect(
        await helpers.isElementVisible(driver, css.resetPassword.resetForm)
      ).to.be.false;
    });

    it('should display error message', async function () {
      expect(
        await helpers.isElementVisible(driver, css.resetPassword.invalidToken)
      ).to.be.true;
    });
  });
  ...
  describe('Matching but expired token', function () {
    before(async function () {
      await helpers.navigateAndWait(driver, expiredResetLink, {
        selector: css.resetPassword.invalidToken,
        timeout: 10000,
      });
    });
  ...
  describe('Valid token', function () {
    before(async function () {
      await helpers.navigateAndWait(driver, resetLink, {
        selector: css.resetPassword.resetForm,
      });
      ...
      function verifyResetForm() {
        ...
        for (let input of Object.keys(inputs)) {
          ...
          it(`should display ${input} field`, async function () {
            expect(await helpers.isElementVisible(driver, inputs[input])).to.be
              .true;
          });
        }
        it('should display submit button', async function () {
          expect(
            await helpers.isElementVisible(driver, css.resetPassword.submit)
          ).to.be.true;
        });
      }
      ...
      describe('Submitting Form', function () {
        ...
        before(async function () {
          await helpers.findInputAndType(
            driver,
            css.resetPassword.inputs.password,
            user.newPassword
          );

          await helpers.findInputAndType(
            driver,
            css.resetPassword.inputs.confirmPassword,
            user.newPassword
          );

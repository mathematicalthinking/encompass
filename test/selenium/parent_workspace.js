...
describe('Parent Workspace creation and updating', function () {
  ...
  let assignmentInfoUrlRegEx = new RegExp(
    `^${host}/assignments/[0-9a-fA-F]{24}$`
  );

  let workspaceInfoUrlRegEx = new RegExp(
    `^${host}/workspaces/[0-9a-fA-F]{24}/info$`
  );
  ...
  function goToWsInfo(url) {
    return helpers.navigateAndWait(driver, url, {
      selector: css.wsInfo.settings.editBtn,
    });
  }
  ...
  async function createAssignment(
    assignmentDetails,
    assignmentNewUrl = `${host}/assignments/new`
  ) {
    ...
    await helpers.navigateAndWait(driver, assignmentNewUrl, {
      selector: css.assignmentsNew.container,
    });

    // select section
    await helpers.selectSingleSelectizeItem(
      driver,
      inputSelectors.section.input,
      section.name,
      section.id
    );
    ...
    await radioButtonSelector.selectRadioButton(
      inputSelectors.linkedWorkspaces.groupName,
      inputSelectors.linkedWorkspaces.yes.value
    );
    ...
    await radioButtonSelector.selectRadioButton(
      inputSelectors.parentWorkspace.groupName,
      inputSelectors.parentWorkspace.yes.value
    );
      ...
      before(async function () {
        ...
        if (options.doUseHref) {
          ...
          await helpers.waitForUrlMatch(
            driver,
            new RegExp(parentWorkspaceHref)
          );
        } else {
          await helpers.navigateAndWait(driver, workspacesUrl, {
            selector: '.results-list',
  ...
  async function createComment(
    ...
    await helpers.selectOption(
      driver,
      css.wsComments.commentTypeSelect,
      commentType,
      true
    );
    ...
    let commentListItems = await helpers.getWebElements(
      driver,
      css.wsComments.commentListItem
    );
  ...
  async function deleteFolderByName(folderName) {
    ...
    await helpers.waitForAndClickElement(
      driver,
      css.workspace.folders.doneEditingIcon
    );
    console.log(2);
  ...
  describe('Resulting linked workspaces', function () {
    ...
    it('should have created a parent workspace', function () {
      ...
      return helpers.waitForNElements(driver, linkSel, 1).then((links) => {
        expect(links).to.have.lengthOf(expectedCount);
        let link = links[0];
        return Promise.all([
          link.getAttribute('href'),
          link.getAttribute('innerText'),
        ]).then((results) => {
    ...
    describe('Visiting one of the linked workspaces', function () {
      it('Should redirect to info page since workspace is empty', function () {
        return linkedWorkspacesLinks[0]
          .click()
          .then(() => {
            return helpers
              .waitForUrlMatch(driver, workspaceInfoUrlRegEx)
              .then((doesMatch) => {
    ...
    describe('Student submitting to linked workspace', function () {
      ...
      before(async function () {
        ...
        return helpers.logout(driver).then(() => {
      ...
      describe('Respond to assignment', function () {
        before(function () {
          return helpers
            .navigateAndWait(driver, assignmentsUrl, {
              selector: '.your-assignments',
            })
            .then(() => {
              return driver
                .findElement(By.linkText(newAssignment.name))
                .then((link) => {
                  return link.click().then(() => {
                    return helpers.waitForSelector(
                      driver,
                      css.assignmentsStudent.infoPage.submitBtn
                    );
                  });
                });
            });
        ...
        it('should create response', async function () {
          ...
          expect(
            await helpers.isElementVisible(
              driver,
              css.assignmentsStudent.newAnswerForm.container
            )
          ).to.be.true;

          await helpers.findInputAndType(
            driver,
            css.assignmentsStudent.newAnswerForm.inputs.briefSummary,
            firstResponse.briefSummary
          );

          await helpers.findInputAndType(
            driver,
            css.assignmentsStudent.newAnswerForm.inputs.explanation,
            firstResponse.explanation
          );

          await helpers.findAndClickElement(
            driver,
            css.assignmentsStudent.newAnswerForm.createBtn
          );
          return true;
        });
  ...
  describe('Checking if parent workspace auto-updated submissions', function () {
    ...
    checkWorkspaceStats(
      teacher.parentWorkspace.name,
      { submissions: 2, folders: teacher.parentWorkspace.initialFolders },
      { doUseHref: true }
    );
  });
  ...
  describe('Creating markup in child workspaces', function () {
    ...
    describe('Marking up as student1', function () {
      ...
      before(async function () {
        ...
        await helpers.findAndClickElement(
          driver,
          wsSelectors.submissionNav.rightArrow
        );
      ...
      it('Creating a comment', async function () {
        ...
        let createdComment = await createComment(
          createdSelection,
          'notice',
          text,
          commentsCount
      ...
      it('Creating a second comment', async function () {
        ...
        secondComment = await createComment(
          createdSelection,
          'feedback',
          text,
          commentsCount + 1
      ...
      it('Deleting second comment', async function () {
        let deleteBtn = await secondComment.findElement({
          css: 'span.delete_button',
        });
        await deleteBtn.click();

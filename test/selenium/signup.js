...
describe('Signup form', function () {
  ...
  before(async function () {
    ...
    await helpers.navigateAndWait(driver, host, {
      selector: signupLink,
  ...
  describe('Displaying form', async function () {
    function verifySignupForm() {
      ...
      it('should display submit button', async function () {
        expect(await helpers.isElementVisible(driver, css.signup.submit)).to.be
          .true;
      });
    }

    it('should display signup form', async function () {
      expect(await helpers.isElementVisible(driver, css.signup.form)).to.be
        .true;
    });
  ...
  describe('Submitting form', function () {
    ...
    it('should remove error when a form field is modified', async function () {
      let usernameInput;
      try {
        usernameInput = await driver.findElement(
    ...
    it('should not allow submitting with invalid password', async function () {
      let passwordInput = await driver.findElement(
        By.css(css.signup.inputs.password)
      );
      passwordInput.clear();
    ...
    it('should display terms error if submitted without checking agree to terms', async function () {
      ...
      await driver.sleep(100);
      expect(await helpers.isTextInDom(driver, helpers.signupErrors.terms)).to
        .be.true;
    });

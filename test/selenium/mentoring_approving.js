...
function checkSortBarDisplay(webDriver, hiddenSelectors = []) {
  ...
  return Promise.all(
    defaults.map((sel) => {
      ...
      return helpers.isElementVisible(webDriver, sel).then((isVisible) => {
...
function checkThreadValues(webDriver, values, excludedValues = []) {
  ...
  return Promise.all(
    valueKeys.map((key) => {
      ...
      return helpers.findAndGetText(webDriver, selector).then((domText) => {
...
describe('Mentoring / Approving Interactions', function () {
  ...
  describe('Navigating to workspace submission', function () {
    ...
    before(async function () {
      ...
      await helpers.navigateAndWait(driver, url, {
        selector: css.workspace.newResponse,
      });
    });
    ...
    it('should display submission view', async function () {
      expect(
        await helpers.findAndGetText(
          driver,
          css.responseInfo.submissionView.studentIndicator
        )
      ).to.eql(feedbackReceiver.username);
    });
    ...
    it('should display mentor reply view', async function () {
      expect(
        await helpers.findAndGetText(
          driver,
          css.responseInfo.mentorReplyView.recipient
        )
      ).to.eql(feedbackReceiver.username);
      ...
      expect(
        await helpers.findAndGetText(
          driver,
          css.responseInfo.mentorReplyView.sender
        )
      ).to.eql(mentorInfo.username);
    });

    it('should display proper buttons', async function () {
      let sendBtn = await helpers.getWebElements(
        driver,
        css.responseInfo.mentorReplyView.saveButton
      );

      expect(await sendBtn[0].getText()).to.eql('Submit for Approval');

      let saveDraftBtn = await helpers.getWebElements(
        driver,
  ...
  describe('Submitting response for approval', function () {
    before(async function () {
      ...
      await helpers.waitForUrlMatch(
        driver,
        /responses\/submission\/[0-9a-f]{24}\?responseId=[0-9a-f]{24}/
      );
    });
    it('should display new reply with pending approval status', async function () {
      let statusText = await helpers.getWebElements(
        driver,
        css.responseInfo.mentorReplyView.statusText
      );
  ...
  describe('Visting Responses List', function () {
    ...
    it('should display mentoring tab and display count', async function () {
      let mentoringTabs = await helpers.getWebElements(
        driver,
        css.responsesList.mentoringTab
      );
    ...
    it('should not display submitter tab', async function () {
      expect(
        await helpers.isElementVisible(driver, css.responsesList.submitterTab)
      ).to.eql(false);
    });
    it('should not display approving tab', async function () {
      expect(
        await helpers.isElementVisible(driver, css.responsesList.approvingTab)
      ).to.eql(false);
    });

    it('should have one response thread in list', async function () {
      let responseThreads = await helpers.getWebElements(
        driver,
        css.responsesList.responseThreads
      );
    ...
    it('should display correct information about thread', async function () {
      let values = {
        ...
        submissionDate: formatDistanceToNow(
          new Date(submissionInfo.createDate),
          { addSuffix: true }
        ),
        replyDate: formatDistanceToNow(new Date(), { addSuffix: true }),
    ...
    it('should indicate that thread has pending reply', async function () {
      let itemContainer = await helpers.getWebElements(
        driver,
        css.responsesList.threadItemContainer
      );
      expect(await itemContainer[0].getCssValue('font-weight')).to.not.eql(
        css.general.boldFontWeight
      );
      ...
      expect(await statusCircle[0].getCssValue('fill')).to.eql(
        css.general.pendingFill
      );

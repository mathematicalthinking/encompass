...
test('visiting the homepage', function () {
  visit('/').then(function () {
    ok(
      find('a[href^="/workspaces"]').length,
      'has a link to the workspaces area'
    );
...
test('clicking homepage workspaces link and then visiting a workspace', function () {
  visit('/').then(function () {
    click('span.al_link>a:contains("workspaces")').then(function () {
      ...
      ok(
        find('span.workspace_name:contains("Period 3")').length,
        'has a link to the period 3 workspace'
      );
      click('span.workspace_name:contains("Period 3")')
        .wait()
        .then(function () {
          ok(
            find('h2:contains("Period 3 Triangle Tops Workspace")').length,
            'the workspace title should display'
          );
          ...
          ok(
            find('.submission_index:contains("1")').length,
            'we should be on the first submission'
          );
          ok(
            find('.submission_count:contains("2")').length,
            'there should be two submissions'
          );
          ok(
            find('div#al_submission div.short:contains("short 1")').length,
            'the first submission should be showing'
          );
          ok(
            find('header>#studentList>li:first:contains("Student 1")').length,
            'the first submission should be Student 1'
          ); //ENC-391, ENC-392
          ok(
            find('header>#studentList>li:last:contains("Ztudent 2")').length,
            'the last submission should be Ztudent 2'
          ); //ENC-391, ENC-392
          equal(
            currentRouteName(),
            'workspace.submission.index',
            'we should be looking at a submission'
          );
          click('div.al_controller>a>span.al_arrow_right').then(function () {
            ok(
              find('.submission_index:contains("2")').length,
              'after clicking the right arrow we should be on the second submission'
            );
            ok(
              find('div#al_submission div.short:contains("222")').length,
              'the second submission should be showing'
            );
          });
        });
    });
...
test('making a selection in a workspace', function () {
  visit('/workspaces/1/submissions/1')
    ...
    .then(function () {
      ...
      selectionHighlighting.createSelectionWithoutMouse(
        'node-1 2 0 node-1 2 5'
      );
      equal(
        selectionHighlighting.getSelection(0).text,
        'short',
    ...
    .then(function () {
      ...
      selectionHighlighting.init();
      selectionHighlighting.createSelectionWithoutMouse(
        'node-1 2 5 node-2 2 1'
      );
      equal(
        selectionHighlighting.getSelection(0).coords,
        'node-1 2 5 node-2 2 1',
    ...
    .then(function () {
      ...
      Encompass.__container__
        .lookup('controller:workspaceSubmission')
        .send('addSelection', {
          id: 5,
    ...
    .then(function () {
      ok(
        find('a.selection_text:contains("bogus selection")').length,
...
test('the make selection checkbox is on by default', function () {
  ...
  andThen(function () {
    ok(
      find('label.makingSelection>input:checked').length,
      'make selections is on'
    );
  });
});
...
test('the make selection checkbox stays on after for other submissions', function () {
  ...
  andThen(function () {
    ...
    ok(
      find('#node-3:contains("italics")').length,
      'the sub-elements of the long answer have been given id attributes'
    ); //ENC-450
  });
});

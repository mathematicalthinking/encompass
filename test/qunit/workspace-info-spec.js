...
test('when visiting your own workpace info page', function () {
  visit('/workspaces/1/info').then(function () {
    ok(
      find('span.owner:contains("amir")').length,
      'the owner username is displayed'
    );
    ok(
      find('button.action_button:contains("Edit")').length,
      'there is an edit button'
    );
    equal(
      find('section.attributes input').length,
      0,
    ...
    click(find('button.action_button:contains("Edit")')).then(function () {
      ...
      fillIn('input.search', 'am').then(function () {
        equal(
          find('button.addEditor').length,
          2,
          'after starting to search buttons will appear to allow you to add the editor'
        );
        click(find('button.addEditor:first')).then(function () {
          equal(
            find('li.editor').length,
            1,
            'after adding an editor, there should be one editor'
          );
          click(find('span.remove_editor:first')).then(function () {
            equal(
              find('li.editor').length,
              0,
              'after removing an editor, there should be zero editors'
            );
            click(find('button.action_button:contains("Save")')).then(
              function () {
                equal(
                  find('section.attributes input').length,
                  0,
                  'after clicking save there are no editable fields'
                );
              }
            );
          });
...
test("when visiting someone else's workpace info page", function () {
  ...
  visit('/workspaces/2/info').then(function () {
    ok(
      find('span.owner:contains("some_trial_user")').length,
      'the owner username is displayed'
    );
    equal(
      find('button.action_button:contains("Edit")').length,

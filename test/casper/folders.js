...
casper.on('resource.error', function (resource) {
  ...
  if (!resource.url.match(/png$/)) {
    casper.log(
      'Resource ' + resource.url + ' failed to load (' + status + ')',
      'error'
    );
...
describe('Folders', function () {
  ...
  describe('Visiting a Folder in Baffling Brother', function () {
    before(function () {
      casper.thenOpen(
        host +
          '/#/workspaces/543e96757112056b290001fa/folders/543ea9837112056b29000ac5'
      );
    ...
    it('should have default view options selected', function () {
      'label#browseByStudent>input'.should.have
        .attribute('checked')
        .and.contain('checked');
      'label#showSubFolders>input'.should.have
        .attribute('checked')
        .and.contain('checked');
      'label#showEvidence>input'.should.have
        .attribute('checked')
        .and.contain('checked');
      'label#showSubmFolders>input'.should.have
        .attribute('checked')
        .and.contain('checked');
    });
    ...
    it('should display a table of submission/selection data', function () {
      ...
      "$('table#folder_contents>tbody>tr').length".should.evaluate.to.be.at.least(
        '13'
      );
      'table#folder_contents>tbody'.should.contain.text('Quotable!');
      //'table#folder_contents>tbody'.should.contain.text('Gabriella');
      'table#folder_contents>tbody'.should.contain.text(
        "If it was a negative it wouldn't work."
      );
    });
  });
});

...
describe('Cache', function () {
  ...
  it('will fail with missing arguments', function (done) {
    ...
    if (promiseA.then) {
      promiseA
        .then(caching.success, caching.failure)
        .then(function () {
          expect(caching.failure).toHaveBeenCalled();
          expect(caching.success).not.toHaveBeenCalled();

          var expected = caching.failure.mostRecentCall.args
            ? caching.failure.mostRecentCall.args[0]
            : null;
  ...
  it('will fail on invalid response', function (done) {
    var promise = cache({
      user: mock.users.user.username,
      teacher: mock.users.user.username,
    });
  ...
  it('will succeed and report on a valid file', function (done) {
    var promise = cache({
      user: mock.users.user.username,
      source: 'test/data/pow/defaultPd.json',
    });
    ...
    if (promise.then) {
      promise
        ...
        .then(function () {
          ...
          var report = caching.success.mostRecentCall.args
            ? caching.success.mostRecentCall.args[0]
            : {};

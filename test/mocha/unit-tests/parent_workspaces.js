...
const {
  generateParentWorkspace,
} = require('../../../server/datasource/api/parentWorkspaceApi');
...
const runTest = function (
  ...
  expectedResults,
  description,
  compareOptions = { doUseDeepEqual: true }
) {
  describe(description, function () {
    ...
    if (compareOptions.doUseDeepEqual) {
      ...
      it('Workspace type should be parent', function () {
        expect(results[1].workspaceType).to.eql(
          expectedResults.parentWorkspace.workspaceType
        );
      });

      it('Should have correct childWorkspaces', function () {
        expect(
          results[1].childWorkspaces.map((id) => id.toString())
        ).to.have.members(expectedResults.parentWorkspace.childWorkspaces);
      });
    }
  });
};
...
describe('Generating a Parent Workspace from a set of child workspaces', function () {
  ...
  describe('Multiple workspaces', async function () {
    ...
    await runTest(config, expectedResults, 'Valid Multiple', {
      doUseDeepEqual: false,
    });
  });
});

...
describe('Creating Parent workspace from assignment', function () {
  ...
  before(async function () {
    ...
    originalLinkedWs = await Promise.all(
      originalLinkedWorkspaceIds.map((id) => {
        return getApiResourceById(agent, 'workspaces', id.toString()).then(
          (results) => results.body.workspace
        );
      })
    );
    results = await putApiResourceById(
      agent,
      'assignments',
  ...
  it('should not have modified linked workspace data', async function () {
    ...
    let populatedLinkedWorkspaces = await Promise.all(
      linkedWorkspaces.map((wsId) => {
        return getApiResourceById(agent, 'workspaces', wsId).then(
          (results) => results.body.workspace
        );
      })
    );

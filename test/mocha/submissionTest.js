    ...
    describe(`Submission CRUD operations as ${user.details.testDescriptionTitle}`, function () {
      ...
      describe('/GET submissions', () => {
        it('should get all submissions', (done) => {
          agent.get(baseUrl).end((err, res) => {
            ...
            expect(res.body.submissions.length).to.eql(
              accessibleSubmissionCount
            );
            done();
          });
      ...
      if (!isStudent) {
        describe('/GET submission by ID', () => {
          it('should get submission', (done) => {
            agent.get(baseUrl + accessibleSubmission._id).end((err, res) => {
              if (err) {
                console.log(err);
              }
              expect(res).to.have.status(200);
      ...
      if (username !== 'actingstudent') {
        describe('Requesting submission that contains superceded responses', function () {
          ...
          if (isStudent) {
            description =
              'Should not return superceded responseId with submission';
          ...
          it(description, function () {
            return agent
              ...
              .then((results) => {
                ...
                expect(responses.includes(supercedResponseId)).to.eql(
                  shouldIncludeSuperceded
                );
              })
              .catch((err) => {
                throw err;

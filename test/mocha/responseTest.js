      ...
      if (actingRole !== 'student') {
        describe('/GET response by ID', () => {
          it('should get response', (done) => {
            agent.get(baseUrl + accessibleResponse._id).end((err, res) => {
              if (err) {
                console.log(err);
              }
              expect(res).to.have.status(200);
        ...
        describe('/PUT update response isTrashed', () => {
          ...
          let responseToTrash = {
            ...
            original:
              '<p>Hello ashleyc,</p><br><p>You wrote:</p><br><blockquote class="pf-response-text">undefined</blockquote><br><p>...and I thought...</p><br><p class="pf-response-text">I\'m not sure about this</p><br>',
            source: 'submission',
            responseType: 'mentor',
            note: null,
            status: 'approved',
            ...
            comments: ['5d5d63384f217a59dfbbdefc'],
            selections: ['5d5d631c4f217a59dfbbdefb'],
          ...
          it('Should have removed response from submission', async () => {
            try {
              await mongoose.connect(
                'mongodb://localhost:27017/encompass_seed',
                { useMongoClient: true }
              );

              let submission = await models.Submission.findById(
                results.body.response.submission
              );

              let subResponses = submission.responses.map((r) => r.toString());
              expect(subResponses).to.not.have.members([
                results.body.response._id,
              ]);

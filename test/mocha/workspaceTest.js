    ...
    describe(`Workspace CRUD operations as ${user.details.testDescriptionTitle}`, function () {
      ...
      describe('/GET workspaces', () => {
        it('should get all workspaces', (done) => {
          agent.get(baseUrl).end((err, res) => {
            ...
            expect(res.body.workspaces.length).to.eql(
              accessibleWorkspacesCount
            );
            done();
          });
      ...
      describe('/GET workspace by ID', () => {
        it('should get workspace', (done) => {
          agent.get(baseUrl + accessibleWorkspace._id).end((err, res) => {
            if (err) {
              throw err;
            }
            expect(res).to.have.status(200);
            ...
            arraysToCheck.forEach((prop) => {
              expect(res.body.workspace[prop]).to.have.members(
                accessibleWorkspace[prop]
              );
            });
            ...
            let sideResponses = _.chain(sideSubmissions)
              ...
              .uniq()
              .value();
            expect(sideResponses).to.have.members(
              accessibleWorkspace.responses
            );

        ...
        it('should post a new answer', (done) => {
          agent
            ...
            .end((err, res) => {
              ...
              expect(res.body.answer.explanation).to.eql(
          ...
          agent
            ...
            .end((err, res) => {
              ...
              if (isAdmin) {
                ...
                expect(res.body.answer).to.have.any.keys('problem', 'answer');
                expect(res.body.answer.explanation).to.eql(
                  fixtures.answer.updated.explanation
                );
                done();
              } else {
      ...
      if (accountType === 'S') {
        describe('/POST revised answer to assignment with a linked workspace', function () {
          ...
          it('should post the revision successfully', function (done) {
            agent
              ...
              .end((err, res) => {
                ...
                expect(answer).to.have.any.keys(
                  'problem',
                  'answer',
                  'priorAnswer'
                );
                expect(meta.updatedWorkspacesInfo).to.have.all.keys(
                ...
                let updatedWorkspaceId =
                  updatedWorkspaceInfo[0].updatedWorkspaceInfo.workspaceId;

                let newSubmissionId =
                  updatedWorkspaceInfo[0].updatedWorkspaceInfo.submissionId;
          ...
          it('should have created a new submission ', async function () {
            try {
              newSubmission = await models.Submission.findOne({
                answer: newAnswerId,
                workspaces: workspaceToUpdate,
              })
                .lean()
                .exec();

              expect(newSubmission).to.exist;
              expect(newSubmission.creator.studentId).to.eql(
                firstRevision.createdBy
              );
            } catch (err) {
              throw err;
            }
          ...
          it('should have added the submission to the workspace', async function () {
            try {
              let workspace = await models.Workspace.findById(workspaceToUpdate)
                .lean()
                .exec();
              expect(
                workspace.submissions.map((sub) => sub.toString())
              ).to.contain(newSubmission._id.toString());

...
function updateResponse(id, putBody, agent) {
  return agent.put(`${baseUrl}${id}`).send(putBody);
}
...
describe('With approver capabilities', function () {
  ...
  describe('Changing status of already approved response', function () {
      ...
      return Promise.all(
        newStatuses.map((status) => {
          it(`should succesfully change status to ${status} `, async function () {
            let res = await updateResponseStatus(
              responseToUpdate._id,
              status,
              agent,
              userId,
              responseToUpdate.text
            );

            if (errorDetails) {
              expect(res.status).to.eql(errorDetails.responseStatus);
              expect(res.body.errors[0].detail).to.eql(
    ...
    updateStatuses(
      alreadyApprovedResponse,
      'Changing status before response has been read by recipient'
    );

    updateStatuses(
      approvedButReadResponse,
      'Changing status after response has been read by recipient',
      {
        responseStatus: 422,
        errorMessage:

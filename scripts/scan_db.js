...
var errors = { count: 0 };
var ERROR = {
  // We use str.replace as a node util.format workaround
  MISSING_COLLECTION: 'Collection %s does not exist',
  BROKEN_RELATIONSHIP:
    '%s(origin) %s(id) has %s(broken) broken %s(endpoint) relationships out of %s(total)',
};
  ...
  for (var origin in rel.objects) {
    ...
    while (iterator.hasNext()) {
      ...
      if (source) {
        ...
        var relatedIds = related.toArray().map(function (obj) {
          return obj._id.toString().replace(idFormat, '');
        }); // Cast to string to enable indexOf comparison
        var hasRelationship = source.hasOwnProperty(endField);
        numRelated = related.count();

        if (hasRelationship) {
          var sourceIds = source[endField].map(function (id) {
            return id.toString().replace(idFormat, '');
          }); // Cast to string to enable indexOf comparison
          ...
          if (!allRelated) {
            var brokenAtSource = relatedIds.filter(function (id) {
              return sourceIds.indexOf(id) < 0;
            });
            var brokenAtTarget = sourceIds.filter(function (id) {
              return relatedIds.indexOf(id) < 0;
            });
            var message = ERROR.BROKEN_RELATIONSHIP.replace(
              '%s(origin)',
              origin
            );
            ...
            if (brokenAtSource.length) {
              details['missing_' + rel.fields.endpoint] = {
                count: brokenAtSource.length,
                ids: brokenAtSource.join(' '),
              };
            }

            if (brokenAtTarget.length) {
              details['missing_' + rel.fields.origin] = {
                count: brokenAtTarget.length,
                ids: brokenAtTarget.join(' '),
              };
            }

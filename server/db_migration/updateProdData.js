...
async function removeTrashedDocuments() {
  ...
  try {
    for (let collection of originalCollections) {
      ...
      trashed = await model.countDocuments({isTrashed: true});
      console.log(`trashed documents to delete count: ${trashed}`)
    }
}catch(err) {
  console.log(err);
}
...
const belongsToWs = ['Selection', 'Comment', 'Response', 'Folder', 'Tagging']
...
async function removeOrphanedFromWs(collection) {
  ...
  console.log(`To be Deleted count ${deleted} for ${collection}s`);
  return deleted;
}
...
async function removeOrphanedWorkspaces() {
  ...
  try {
    ...
    let deleted = 0;
    const model = models.Submission;
    const subs = await model.find({});
    console.log(`there are ${subs.length} submissions`);

    for (sub of subs) {
      const shouldDelete = false;
      const wsIds = sub.workspaces;
      const newWsIds = wsIds.filter((el) => {
        return workspaceIds.includes(el);
      });
      if (wsIds.length !== newWsIds.length) {
        deleted++
      }
    }
  }catch(err) {
  ...
  console.log(`submissions with changed workspaceIds count: ${deleted}`);
  // return results;
}
...
async function removeOrphanedSubs() {
  try {
    ...
    let submissionIds = submissions.map(sub => sub._id);
    // results = await models.Workspace.updateMany({}, {$pull: {submissions: {$nin: submissionIds}}});
    // count orphaned submissions
    let deleted = 0;
    const model = models.Workspace;
    const workspaces = await model.find({});
    console.log(`there are ${workspaces.length} workspaces`);

    for (workspace of workspaces) {
      const shouldDelete = false;
      const subIds = workspace.submissions;
      const newSubIds = subIds.filter((el) => {
        return subIds.includes(el);
      });
      if (subIds.length !== newSubIds.length) {
        deleted++
      }
    }
  }catch(err) {
...
// set accountType for all users (and remove isAdmin field).
...
async function updateData() {
  try {
    ...
    // to do - set common fields in all files (isTrasned, created and mod dates)
  }catch(err) {
    console.log(err);
  }
  console.log('done!');

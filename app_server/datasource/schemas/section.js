...
SectionSchema.post('save', function (Section) {
  ...
  let update = {
    $addToSet: { sections: { sectionId: SectionIdObj, role: 'teacher' } },
  };

  if (Section.isTrashed) {
    /* + If deleted, all references are also deleted */
    update = {
      $pull: { sections: { sectionId: SectionIdObj, role: 'teacher' } },
    };
  }
  ...
  if (Section.students) {
    let studentUpdate = {
      $addToSet: { sections: { sectionId: SectionIdObj, role: 'student' } },
    };

    if (Section.isTrashed) {
      /* + If deleted, all references are also deleted */
      studentUpdate = {
        $pull: { sections: { sectionId: SectionIdObj, role: 'student' } },
      };

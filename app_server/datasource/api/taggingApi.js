...
async function getTaggings(req, res, next) {
  ...
  models.Tagging.find(criteria).exec(function (err, tags) {
    if (err) {
      logger.error(err);
...
async function getTagging(req, res, next) {
  try {
    ...
    if (!canLoadTagging) {
      // user does not have permission to access tagging
...
async function postTagging(req, res, next) {
  try {
    ...
    let canCreateTaggingInWs = wsAccess.canModify(
      user,
      workspace,
      'selections',
      2
...
function putTagging(req, res, next) {
  ...
  models.Workspace.findById(workspaceId)
    ...
    .exec(function (err, ws) {
      ...
      if (
        areObjectIdsEqual(user.id, req.body.tagging.createdBy) ||
        wsAccess.canModify(user, ws, 'selections', 3)
      ) {
        models.Tagging.findById(req.params.id, function (err, doc) {
          if (err) {
            logger.error(err);
            return utils.sendError.InternalError(err, res);
          }

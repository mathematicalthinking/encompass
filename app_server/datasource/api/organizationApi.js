...
const getOrganization = (req, res, next) => {
  models.Organization.findById(req.params.id).exec((err, organization) => {
    if (err) {
      logger.error(err);
      return utils.sendError.InternalError(err, res);
    }
...
function canModifyOrg(user, orgId) {
  ...
  if (accountType === 'A') {
    ...
    accountType === 'P' &&
    JSON.stringify(userOrg) === JSON.stringify(orgId)
  ) {
    return true;
  }
}
...
const postOrganization = (req, res, next) => {
  ...
  if (!canPost) {
    return utils.sendError.NotAuthorizedError(
      'You are not authorizd to create a new organization.',
      res
    );
  }

  return apiUtils
    .isRecordUniqueByStringProp(
      'Organization',
    ...
    .catch((err) => {
      if (err.message === 'duplicateName') {
        return utils.sendError.ValidationError(
          `There is already an existing organization named "${req.body.organization.name}."`,
          'name',
          res
        );
      }

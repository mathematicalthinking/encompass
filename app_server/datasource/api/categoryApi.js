...
const getCategories = (req, res, next) => {
  if (req.query.ids) {
    ...
    models.Category.find(criteria).exec((err, categories) => {
      if (err) {
        logger.error(err);
    ...
    models.Category.find({ identifier: { $eq: req.query.identifier } }).exec(
      (err, categories) => {
        if (err) {
          logger.error(err);
          return utils.sendError.InternalError(err, res);
    ...
    models.Category.find({ identifier: regex }).exec((err, categories) => {
      if (err) {
        logger.error(err);
        return utils.sendError.InternalError(err, res);
      }
...
const getCategory = (req, res, next) => {
  models.Category.findById(req.params.id).exec((err, category) => {
    if (err) {
      logger.error(err);
      return utils.sendError.InternalError(err, res);
    }

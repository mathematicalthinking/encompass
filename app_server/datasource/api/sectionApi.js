...
const postSection = (req, res, next) => {
  ...
  section.save((err, doc) => {
    ...
    models.User.updateMany(
      ...
      {
        $addToSet: {
          sections: { sectionId: doc._id, role: 'teacher' },
        },
      }
    ).exec();
...
const putSection = async (req, res, next) => {
  try {
    ...
    let originalTeacherStringIds = originalTeacherIds.map((id) =>
      id.toString()
    );
    ...
    if (isNonEmptyArray(removedStudents)) {
      let updateHash = {
        $pull: { sections: { sectionId: savedSection._id, role: 'student' } },
      };
    ...
    if (isNonEmptyArray(addedStudents)) {
      let updateHash = {
        $addToSet: {
          sections: { sectionId: savedSection._id, role: 'student' },
        },
      };
      ...
      if (isNonEmptyArray(sectionAssignments)) {
        updateHash = {
          $addToSet: {
            sections: { sectionId: savedSection._id, role: 'student' },
            assignments: { $each: sectionAssignments },
          },
        };

        models.Assignment.updateMany(
          { _id: { $in: sectionAssignments } },
    ...
    if (isNonEmptyArray(removedTeachers)) {
      models.User.updateMany(
        ...
        {
          $pull: { sections: { sectionId: savedSection._id, role: 'teacher' } },
        }
      ).exec();
    }
    ...
    if (isNonEmptyArray(addedTeachers)) {
      models.User.updateMany(
        ...
        {
          $addToSet: {
            sections: { sectionId: savedSection._id, role: 'teacher' },
          },
        }
      ).exec();
    }

...
const postVmtImportRequests = async (req, res, next) => {
  try {
    ...
    let {
      ...
      workspaceName,
      folderSet,
    } = req.body.vmtImportRequest;

    let roomsWithProblems = await createDefaultProblemsFromVmtRooms(
      user,
      vmtRooms
    );
    ...
    let submissions = await convertVmtAnswersToSubmissions(
      answerRecords,
      roomsWithProblems
    );
    let workspace = new models.Workspace({
    ...
    if (isValidMongoId(folderSet)) {
      ...
      savedWorkspace = await models.Workspace.findById(
        savedWorkspace._id
      ).lean();
    }

...
async function getSubmissions(req, res, next) {
  ...
  try {
    ...
    let criteriaPromises = [
      access.get.submissions(user, req.query.ids, req.query.filterBy),
    ];

    if (doFilterResponses) {
      criteriaPromises.push(
        responseAccess.get.responses(user, null, null, {
          submissions: req.query.ids,
        })
      );
    }
    let [submissionCriteria, responseCriteria] = await Promise.all(
      criteriaPromises
    );

    let recordPromises = [
      models.Submission.find(submissionCriteria).lean().exec(),
    ];

    if (responseCriteria) {
      recordPromises.push(
        models.Response.find(responseCriteria, { _id: 1 }).lean().exec()
      );
    }
...
/**
 * @public
 * @method findSubmissionsByAnswerId
 * @description finds all submissions that use the same answer
 */

async function findSubmissionsByAnswerId(answer) {
  const submissions = await models.Submission.find({ answer });
  return submissions;
}
...
async function getSubmission(req, res, next) {
  try {
    ...
    if (!submission || submission.isTrashed) {
    ...
    if (!canLoadSubmission) {
      // user does not have permission to access submission
    ...
    if (doFilterResponses) {
      ...
      if (isNonEmptyArray(responseIds)) {
        responseCriteria = await responseAccess.get.responses(
          user,
          responseIds
        );
        let responses = await models.Response.find(responseCriteria, { _id: 1 })
          .lean()
          .exec();
...
function postSubmission(req, res, next) {
  ...
  if (user.isAuthorized || isAnonymousPost) {
    var access =
      utils.isValidApiKey(req.headers.secret, req.headers.time) ||
      user.isAuthorized;
...
function importSubmissions(req, res, next) {
  ...
  for (var field in params) {
    if (
      !params[field] ||
      (_.isArray(params[field]) && _.isEmpty(params[field]))
    ) {
      delete params[field];
    }
  }
  ...
  var buildWorkspaces = spaces.post.newWorkspaceRequest.bind(
    this,
    req,
    res,
    next

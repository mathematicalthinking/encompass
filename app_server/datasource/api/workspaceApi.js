  ...
  if (_.propertyOf(submissions)('all') !== true) {
    ...
    if (selections === 0) {
      ...
      const subResponses = _.chain(ws.responses)
        .filter(
          (res) =>
            res.submission && submissionIds.includes(res.submission.toString())
        )
        .flatten()
        .value();
...
function getWorkspace(
  ...
  models.Workspace.findById(id)
    ...
    .exec(function (err, ws) {
      ...
      const restrictedDataMap = getRestrictedDataMap(
        user,
        specialPermissions,
        ws
      );
...
function getWorkspaceWithDependencies(id, callback) {
  ...
  models.Workspace.findById(id).exec(function (err, workspace) {
    if (err) {
      throw err;
    }
    if (!workspace) {
      logger.info('No such workspace');
      callback();
      return;
    }
    models.Folder.find(/**{'_id': { $in: workspace.folders}}*/).exec(function (
      err,
      folders
    ) {
      if (err) {
      ...
      models.Submission.find({
        _id: { $in: workspace.submissions },
        isTrashed: false,
      }).exec(function (err, submissions) {
        if (err) {
          throw err;
        }
        models.Selection.find({
          _id: { $in: workspace.selections },
          isTrashed: false,
          ...
          models.Comment.find({
            ...
            models.Tagging.find({
              _id: { $in: workspace.taggings },
              isTrashed: false,
            }).exec(function (err, taggings) {
              if (err) {
                throw err;
              }
              callback({
...
async function resolvePermissionObject(user, ws) {
  try {
    ...
    let permissionObject = _.find(permissions, (obj) =>
      areObjectIdsEqual(user._id, obj.user)
    );
    ...
    if (isValidMongoId(ws.linkedAssignment)) {
      let assignment = await models.Assignment.findById(ws.linkedAssignment)
        .lean()
        .exec();
      ...
      if (assignment) {
        ...
        let foundSection = _.find(userSections, (sectionObj) => {
          return (
            sectionObj.role === 'teacher' &&
            areObjectIdsEqual(sectionObj.sectionId, assignment.section)
          );
        });
  ...
  return Promise.all(
    _.map(results, (ws) => {
      return resolvePermissionObject(user, ws).then((results) => {
        ...
        let areSubmissionRestrictions =
          _.propertyOf(permissionObject)(['submissions', 'all']) !== true ||
          isBasicStudentAccess;

        let isApprover =
          _.propertyOf(permissionObject)('feedback') === 'approver';
        let fetchedWs;
        ...
        return fetchedWs.then((populatedWs) => {
          let populatedResponses = populatedWs.responses
            ? populatedWs.responses.slice()
            : [];
          // eslint-disable-next-line no-unused-vars

          if (areSubmissionRestrictions) {
            const [canLoad, specialPermissions] = access.get.workspace(
              user,
              populatedWs
            );
            const restrictedDataMap = getRestrictedDataMap(
          ...
          ws.responses = _.chain(populatedResponses)
            .filter((response) => {
              ...
              if (areSubmissionRestrictions) {
                ...
                if (
                  !_.find(ws.responses, (responseId) => {
...
async function sendWorkspace(req, res, next) {
  try {
      ...
      if (!isLinkedAssignmentTeacher) {
        ...
        let ownSub = _.find(ws.submissions, (sub) => {
          return areObjectIdsEqual(
            _.propertyOf(sub)(['creator', 'studentId']),
            user._id
          );
        });
    ...
    ws.submissions.forEach((sub) => {
      ...
      if (
        isNonEmptyObject(user) &&
        user.username !== 'old_pows_user' &&
        !submissionUsers[user._id]
      ) {
    ...
    _.keys(relatedData).forEach(function (key) {
      if (ws[key]) {
        ...
        ws[key].forEach(function (item) {
          ...
          let originalRecordProp = `original${stringUtils.capitalizeWord(
    ...
    if (isNonEmptyArray(ws.childWorkspaces)) {
      ...
      data.workspaces = Array.isArray(data.workspaces)
        ? data.workspaces.concat(ws.childWorkspaces)
        : ws.childWorkspaces;
  ...
  try {
    ...
    if (mode === 'public' || mode === 'internet') {
      let isNameUnique = await apiUtils.isRecordUniqueByStringProp(
        ...
        name,
        'name',
        { _id: { $ne: req.params.id }, mode: { $in: ['public', 'internet'] } }
      );

      if (!isNameUnique) {
        return utils.sendError.ValidationError(
          `There is already an existing public workspace named "${name}."`,
          'name',
          res
        );
      }
    ...
    let isCreator = areObjectIdsEqual(user._id, ws.createdBy);
    let isPdWs =
      isPdAdmin && areObjectIdsEqual(user.organization, ws.organization);
    ...
    let isApprover =
      ownPermissionObj && ownPermissionObj.feedback === 'approver';
    ...
    if (_.isArray(wsPermissions)) {
      ...
      if (isNonEmptyArray(removedCollabs)) {
        models.User.updateMany(
          { _id: { $in: removedCollabs } },
          { $pull: { collabWorkspaces: savedWorkspace._id } }
        ).exec();
      }
    }
...
function updateWorkspaces(workspaces, submissionSet) {
  ...
  var updateWorkspace = function (err, ws) {
    ...
    models.Submission.populate(
      submissionSet,
      { path: 'submissions', model: 'Submission' },
      function (err, subset) {
        if (err) {
          logger.error(err);
          return;
        ...
        var transformExistingSubmissions = Q.all(
          ws.submissions.map(function (obj) {
            var sub = obj.toJSON({ transform: insertPowId });
            return sub;
          })
        );

        var transformNewSubmissions = Q.all(
          subset.submissions.map(function (obj) {
            var sub = obj.toJSON({ transform: insertPowId });
            return sub;
          })
        );
        ...
        transformExistingSubmissions.then(function (old) {
          transformNewSubmissions.then(function (newer) {
            ...
            var startDates = [
              ws.submissionSet.description.firstSubmissionDate,
              _.first(sorted).createDate,
            ];
            var endDates = [
              ws.submissionSet.description.lastSubmissionDate,
              _.last(sorted).createDate,
            ];
  ...
  workspaces.forEach(function (workspace) {
    tasks.push(
      models.Workspace.findById(workspace._id)
        .populate('submissions')
...
async function newFolderStructure(user, wsInfo, folderSetHash) {
  try {
    ...
    if (isNonEmptyArray(folderSetObjects)) {
      ...
      const folderSet = await models.FolderSet.findById(folderSetId)
        .lean()
        .exec();
    ...
    const recurse = async function (folderObj, parentId) {
      try {
        ...
        if (_.isEmpty(children)) {
          return f.save().then((doc) => {
            folderIds.push(doc._id);
            return doc;
          });
        }

        f.children = await Promise.all(
...
function nameWorkspace(submissionSet, user, isPows) {
  ...
  var mask = function (source) {
    return pdSet === 'default' ? 'PoWs' : pdSet;
  };

  var name = group
    ? helper.format(labelFmt, puzzle.title, group.name, mask(pdSet))
...
function prepareUserSubmissions(user, pdSet, folderSet, callback) {
  ...
  models.Submission.find(
    { 'teacher.username': user.username, pdSet: pdSet },
    function (err, docs) {
      if (err) {
        logger.error(err);
        callback(err);
...
function sendWorkspaces(req, res, next) {
  ...
  prepareAndUpdateWorkspaces(user, function (err) {
    ...
    models.Workspace.find(userAuth.accessibleWorkspacesQuery(user)).exec(
  ...
  prepareUserSubmissions(user, pdSetName, folderSetId, function () {
    ...
    packageSubmissions(
      ...
      function (err, results) {
        ...
        handleSubmissionSets(submissionSets, user, folderSetId).then(
          function () {
...
async function getWorkspaces(req, res, next) {
  ...
  if (filterBy) {
    ...
    if (includeFromOrg && user.organization) {
      let userIdsFromOrg = await accessUtils.getModelIds('User', {
    ...
    if (criterion) {
      if (criterion === 'all') {
        ...
        let [ownerIds, editorIds] = await Promise.all([
          ...
          apiUtils.filterByForeignRefArray(
            'Workspace',
            query,
            'editors',
            'username'
        ...
        let ids = await apiUtils.filterByForeignRef(
          'Workspace',
          query,
          'owner',
          'username'
        ...
        let ids = await apiUtils.filterByForeignRefArray(
          'Workspace',
          query,
          'editors',
          'username'
  ...
  const criteria = await access.get.workspaces(
    user,
    ids,
    filterBy,
    searchFilter,
  ...
  try {
    ...
    if (teacher && !isUserTeacher) {
      ...
      let areValidAssignments =
        _.isArray(assignments) && !_.isEmpty(assignments);
        ...
        if (student.username !== 'old_pows_user') {
          ...
          if (isNonEmptyArray(studentNames)) {
            if (studentNames.length === 1) {
              ...
              creator.safeName = apiUtils.getSafeName(
                creator.fullName,
                false,
                false
              );
  ...
  try {
    ...
    if (mode === 'public' || mode === 'internet') {
      let isNameUnique = await apiUtils.isRecordUniqueByStringProp(
        ...
        requestedName,
        'name',
        { mode: { $in: ['public', 'internet'] } }
      );

      if (!isNameUnique) {
        let rec = pruned;
        rec.createWorkspaceError =
          'There already exists a public workspace with that name';
...
async function copyAndSaveFolderStructure(
  user,
  originalWsId,
  folderIds,
  folderSetOptions
  ...
  try {
    ...
    const workspace = await models.Workspace.findById(originalWsId)
      .populate('folders')
      .lean()
      .exec();
    ...
    const topLevelFolders = _.filter(folders, (folder) => {
      return (
        isNil(folder.parent) &&
        _.contains(
          _.map(folderIds, (id) => id.toString()),
          folder._id.toString()
        )
      );
    });
    ...
    const recurse = function (folderId) {
      ...
      return models.Folder.findById(folderId)
        .lean()
        .exec()
        .then(async (doc) => {
          if (_.isNull(doc)) {
          ...
          ret.children = await Promise.all(
            _.map(children, (child) => {
    ...
    const folderObjects = await Promise.all(
      _.map(topLevelFolderIds, (folderId) => {
...
*/
  return models.Tagging.findById(id)
    .lean()
    .exec()
    .then(async (tagging) => {
      if (_.isNull(tagging)) {
      ...
      await models.Selection.update(
        { _id: taggingsKey[tagging._id] },
        { $pull: { taggings: tagging._id } }
      );
...
async function deepCloneFolders(user, folderIds, taggingsKey, wsInfo) {
  ...
  try {
    ...
    const oldFolders = await models.Folder.find({ _id: { $in: folderIds } })
      .lean()
      .exec();
...
async function buildTaggingsKey(folderIdsToCopy, selectionsKey) {
  ...
  const folders = await models.Folder.find({ _id: { $in: folderIdsToCopy } })
    .populate('taggings')
    .lean()
    .exec();
  if (_.isEmpty(folders)) {
...
async function handleNewFolders(
  user,
  wsInfo,
  oldFolderIds,
  options,
  ...
  try {
    ...
    */
    ...
    if (none || !areOldIdsToCopy) {
      ...
      if (
        !isNonEmptyObject(folderSetOptions) ||
        isNil(folderSetOptions.existingFolderSetToUse)
      ) {
        return results;
      }
      ...
      if (folderSetOptions.existingFolderSetToUse) {
        folderSetInfo = {
          folderSetId: folderSetOptions.existingFolderSetToUse,
        };
      }
      const newFolderIds = await newFolderStructure(
        user,
        wsInfo,
        folderSetInfo
      );
    ...
    if (includeStructureOnly) {
      ...
      */
      let folderSetInfo = await copyAndSaveFolderStructure(
        user,
        originalWsId,
        folderIdsToCopy,
        folderSetOptions
      ...
      let [folderIds, taggingIds] = await deepCloneFolders(
        user,
        folderIdsToCopy,
        taggingsKey,
        wsInfo
...
function copyComment(oldCommentId, newSub, selectionsKey, newWsId) {
  return models.Comment.findById(oldCommentId)
    .lean()
    .exec()
    .then((oldComment) => {
...
function copySelection(user, oldSelId, newSubId, newWsId, selectionsKey) {
  ...
  return models.Selection.findById(oldSelId)
    .lean()
    .exec()
    .then((oldSelection) => {
      // sel does not exist
...
async function buildCommentsKey(
  selectionsKey,
  originalCommentIds,
  commentsConfig
) {
...
function copyResponse(originalResponseId, newSubId, newWsId) {
  return models.Response.findById(originalResponseId)
    .lean()
    .exec()
    .then((oldResponse) => {
...
// check if single user making a copy of a workspace or teacher making batch copies for class
const cloneWorkspace = (req, res) => {
  if (req.body.copyWorkspaceRequest.batchClone) {
    return batchCloneWorkspace(req, res);
  } else {
    return cloneSingleWorkspace(req, res);
  }
};
...
const batchCloneWorkspace = async (req, res = {}) => {
  ...
  //make batch copies for groups within classes
  if (copyWorkspaceRequest.batchClone.mode === 'group') {
  ...
  if (copyWorkspaceRequest.batchClone.createParent) {
    const name = `Parent Workspace: ${copyWorkspaceRequest.name}`;
    const parentOptions = {
...
async function cloneSingleWorkspace(req, res, next) {
  try {
    ...
    if (!isNonEmptyObject(copyWorkspaceRequest)) {
      return utils.sendError.InvalidContentError(
        'Invalid or missing copy workspace request parameters',
        res
      );
    }
    ...
    */
    ...
    const originalWs = await models.Workspace.findById(originalWsId)
      .populate({ path: 'submissions', populate: { path: 'answer' } })
      .populate('answers')
      .lean()
      .exec();
    ...
    if (folderSetOptions) {
      if (
        folderSetOptions.doCreateFolderSet &&
        folderSetOptions.privacySetting === 'E'
      ) {
        // ensure name is unique
        let isNameUnique = await apiUtils.isRecordUniqueByStringProp(
          'FolderSet',
          folderSetOptions.name,
          'name',
          { privacySetting: 'E' }
        );
    ...
    */
    ...
    let submissionsToClone = buildSubmissionsToClone(
      submissionSet,
      submissionOptions
    );
    ...
    */
    const selectionsKey = buildSelectionsKey(
      submissionSels,
      originalWs.selections,
      selectionOptions
    );
    ...
    */
    const commentsKey = await buildCommentsKey(
      selectionsKey,
      originalWs.comments,
      commentOptions
    );
    ...
    const responsesKey = buildResponsesKey(
      originalWs.responses,
      responseOptions
    );

    const newSubsWithResponses = await Promise.all(
      _.map(newSubsWithComments, (submission) => {
        return copyResponses(responsesKey, submission, newWs._id).then(
          (responseIds) => {
    ...
    const newSubmissionSet = await importApi.buildSubmissionSet(
      newSubsWithSelections,
      user
    );
  ...
  */
    const newFolders = await handleNewFolders(
      user,
      workspaceInfo,
      originalWs.folders,
      folderOptions,
    ...
    if (isNonEmptyObject(permissionOptions)) {
      if (isNonEmptyArray(permissionOptions.permissionObjects)) {
        ...
        await models.User.updateMany(
          ...
          { $addToSet: { collabWorkspaces: newWs._id } }
        );
...
async function addAnswerToWorkspace(user, answer, workspaceId, options = {}) {
  try {
    if (
      !isNonEmptyObject(user) ||
      !isNonEmptyObject(answer) ||
      !isValidMongoId(workspaceId)
    ) {
      return;
    }
    ...
    workspaceToUpdate.submissions = workspaceToUpdate.submissions.map(
      (sub) => sub._id
    );
    workspaceToUpdate.submissions = workspaceToUpdate.submissions.concat([
      savedSub._id,
    ]);
...
function addAnswerToWorkspaces(user, answer) {
  ...
  return Promise.all(
    ...
    if (isNonEmptyArray(updatedWorkspaceIds)) {
      let parentWorkspacesToUpdate = await models.Workspace.find({
        isTrashed: false,
        childWorkspaces: { $elemMatch: { $in: updatedWorkspaceIds } },
        doAutoUpdateFromChildren: true,
      })
        .populate('submissions')
        .exec();
      ...
      return Promise.all(
        parentWorkspacesToUpdate.map((parentWs) => {
          return addAnswerToWorkspace(user, answer, parentWs._id, {
            isParentWsUpdate: true,
          });
        })
      ).then((parentResults) => {
...
const updateWorkspaceRequest = async function (req, res, next) {
  try {
    ...
    let newUpdateRequest = new models.UpdateWorkspaceRequest(
    ...
    if (
      ...
      (!isValidMongoId(linkedAssignment) && !isParentUpdate)
    ) {
    ...
    if (isParentUpdate) {
      popWorkspace = await models.Workspace.findById(workspace)
        .populate('submissions')
      ...
      [popWorkspace, assignment] = await Promise.all([
        models.Workspace.findById(workspace)
          ...
          .populate('createdBy')
          .exec(),
        models.Assignment.findById(linkedAssignment)
          .populate('answers')
          .lean()
          .exec(),
      ]);
    ...
    if (!access.canUpdateSubmissions(user, popWorkspace, 'bulk')) {
      return utils.sendError.NotAuthorizedError(
        "You do not have permission to update this workspace's submissions",
        res
      );
    }
    ...
    if (isParentUpdate) {
      let results = await parentWsApi.updateParentWorkspace(
        user,
        popWorkspace,
        newUpdateRequest
      );
      ...
      let updatedWorkspace = await models.Workspace.findById(popWorkspace._id)
        .lean()
        .exec();
      ...
      if (!isNonEmptyArray(JSONObjects)) {
        newUpdateRequest.updateErrors.push(
          'Error updating workspace with new submissions.'
        );
        await newUpdateRequest.save();
        return utils.sendResponse(res, {
          updateWorkspaceRequest: newUpdateRequest,
      ...
      let savedSubs = await Promise.all(
        JSONObjects.map((obj) => {
          obj.createDate = Date.now();
          let creatorId;

...
function getResponse(req, res, next) {
  ...
  return models.Response.findById(req.params.id)
    ...
    .populate({
      path: 'workspace',
      populate: [{ path: 'owner' }, { path: 'createdBy' }],
    })
    .exec()
    .then((response) => {
...
function getResponses(req, res, next) {
  ...
  return access.get
    .responses(user, ids, workspace, filterBy, isAdminActingPd)
    .then((criteria) => {
      models.Response.find(criteria)
        .lean()
        .exec()
        .then((responses) => {
...
async function postResponse(req, res, next) {
  try {
    ...
    if (!wsAccess.canModify(user, popWs, 'feedback', 1)) {
      return utils.sendError.NotAuthorizedError(
        'You are not authorized to create responses for this workspace.',
        res
      );
    }
    ...
    if (!Array.isArray(parsedResponseTextEls)) {
      return utils.sendError.InvalidContentError(
        'Missing or invalid response text provided',
        res
      );
    }

    // create image record for every base64 image (resize if necessary)
    let convertedResponseText = await apiUtils.handleBase64Images(
      parsedResponseTextEls,
      user
    );
...
async function putResponse(req, res, next) {
  try {
    ...
    if (didStatusChange && isRead) {
      ...
      let errorMsg =
        "A response's status cannot be changed once it has been read by its recipient";
      ...
      return models.Workspace.aggregate([
          ...
          $project: {
            ...
            submissions: {
              $filter: {
                input: '$submissions',
                as: 's',
                cond: { $eq: ['$$s.creator.studentId', userIdStr] },
              },
            },
          ...
          $project: {
            ...
            unreadResponses: {
              $filter: {
                input: '$responses',
                as: 'r',
                cond: { $eq: ['$$r.wasReadByRecipient', false] },
              },
            },
          },
        },
        ...
        { $sort: { 'responses.createDate': -1 } },
        {
          $group: {
            ...
            wsName: { $first: '$wsName' },
            unreadReplies: { $first: '$unreadResponses' },
            mentors: { $addToSet: '$responses.createdBy' },
            responseIds: { $push: '$responses._id' },
        ...
        { $sort: { 'submissions.createDate': -1 } },
        {
          $group: {
            ...
            wsName: { $first: '$wsName' },
            unreadReplies: { $first: '$unreadReplies' },
            responseIds: { $first: '$responses._id' },
            submissions: { $push: '$submissions' },
            submissionIds: { $push: '$submissions._id' },
            mentors: { $first: '$mentors' },
            problemTitle: { $first: '$submissions.publication.puzzle.title' },
          },
        },

        {
          $project: {
            ...
            submissions: 1,
            hasUnreadReply: {
              $cond: [{ $eq: [{ $size: '$unreadReplies' }, 0] }, false, true],
            },
            ...
            paginatedResults: [
              {
                $sort: {
                  hasUnreadReply: 1,
                  'latestReply.createDate': 1,
                  'latestSubmission.createDate': 1,
                },
              },
              { $skip: skip },
              { $limit: limit },
      ...
      return models.Workspace.aggregate([
        ...
        { $unwind: '$submissions' },
        {
          $project: {
            ...
            student: {
              $ifNull: [
                '$submissions.creator.studentId',
                '$submissions.creator.username',
              ],
            },
            studentDisplay: {
              $ifNull: [
                '$submissions.creator.username',
                '$submissions.creator.safeName',
              ],
            },
            submissions: 1,
            responses: 1,
        ...
        {
          $group: {
            _id: { workspaceId: '$workspaceId', student: '$student' },
            studentDisplay: { $first: '$studentDisplay' },
            name: { $first: '$name' },
            submissions: { $push: '$submissions' },
            submissionIds: { $push: '$submissions._id' },
            responses: { $first: '$responses' },
          ...
          $project: {
            ...
            responses: {
              $filter: {
                input: '$responses',
                as: 'r',
                cond: { $in: ['$$r.submission', '$submissionIds'] },
              },
            },
          },
        },
        ...
        {
          $group: {
            _id: { workspaceId: '$_id.workspaceId', student: '$_id.student' },
            responses: { $push: '$responses' },
            name: { $first: '$name' },
            studentDisplay: { $first: '$studentDisplay' },
            submissions: { $first: '$submissions' },
            responseIds: { $push: '$responses._id' },
            submissionIds: { $first: '$submissionIds' },
          },
        },
        { $unwind: '$submissions' },
        { $sort: { 'submissions.createDate': -1 } },
...

        {
          $project: {
            ...
            submissions: 1,
            latestReply: { $arrayElemAt: ['$responses', 0] },
            latestRevision: { $arrayElemAt: ['$submissions', 0] },
            submissionIds: 1,
            responseIds: 1,
            toUserResponses: {
              $filter: {
        ...
        },
        {
          $project: {
            ...
            submissions: 1,
            submissionIds: 1,
            latestReplyDate: '$latestReply.createDate',
            latestRevisionDate: '$latestRevision.createDate',
            hasUnread: {
              $gte: [
                {
                  $size: {
                    $filter: {
                      input: '$toUserResponses',
                      as: 'r',
                      cond: { $eq: ['$$r.wasReadByRecipient', false] },
                    },
                  },
              ...
              $gte: [
                {
                  $size: {
                    $filter: {
                      input: '$fromUserResponses',
                      as: 'r',
                      cond: { $eq: ['$$r.status', 'pendingApproval'] },
                    },
                  },
                },
                1,
              ...
              $gte: [
                {
                  $size: {
                    $filter: {
                      input: '$fromUserResponses',
                      as: 'r',
                      cond: { $eq: ['$$r.status', 'needsRevisions'] },
                    },
                  },
                },
                1,
...
function getApproverThreads(user, asSuperAdmin, limit, skip) {
  ...
  wsCriteria = {
    ...
    $or: [
      ...
      {
        permissions: {
          $elemMatch: { $and: [{ user: userId }, { feedback: 'approver' }] },
        },
      },
    ],
  };
  ...
  return models.Workspace.aggregate([
    ...
    {
      $group: {
        _id: { workspaceId: '$_id', student: '$student' },
        name: { $first: '$name' },
        submissions: { $push: '$submissions' },
        submissionIds: { $push: '$submissions._id' },
        responses: { $first: '$responses' },
    ...
    { $unwind: '$mentorResponses' },
    {
      $group: {
        ...
        submissionIds: { $first: '$submissionIds' },
        responses: { $first: '$responses' },
        mentors: { $addToSet: '$mentorResponses.createdBy' },
        mentorResponses: { $push: '$mentorResponses' },
    ...
    { $unwind: '$mentors' },
    {
      $group: {
        ...
        submissions: { $first: '$submissions' },
        responses: { $first: '$responses' },
        mentorResponses: { $first: '$mentorResponses' },
        submissionIds: { $first: '$submissionIds' },
        problemTitle: { $first: '$problemTitle' },
        studentDisplay: { $first: '$studentDisplay' },
      },
...
async function getResponseThreads(req, res, next) {
  try {
    ...
    if (threadType === 'all') {
      ...
      let mentoringThreads = [{ paginatedResults: [] }, { totalCount: [] }];
      let approvingThreads = [{ paginatedResults: [] }, { totalCount: [] }];
      ...
      let submitterThreads = [{ paginatedResults: [] }, { totalCount: [] }];
      let approvingThreads = [{ paginatedResults: [] }, { totalCount: [] }];
      threads = [submitterThreads, mentoringThreads, approvingThreads];
    } else if (threadType === 'approver') {
      let approvingThreads = await getApproverThreads(
        user,
        asSuperAdmin,
        limit,
        skip
    ...
    if (isNonEmptyArray(mentoringThreads)) {
      mentoringThreads.forEach((thread) => {
        ...
        if (wsId && student) {
          ...
          thread._id = id;
        } else if (thread.uniqueIdentifier.wsId) {
          thread._id = thread.uniqueIdentifier.workspaceId;
        }
        results.push(thread);
      });
    }
    if (isNonEmptyArray(approvingThreads)) {
      approvingThreads.forEach((thread) => {
        ...
        if (wsId && student && mentor) {
          ...
          thread._id = id;
        } else if (thread.uniqueIdentifier.workspaceId) {
          thread._id = thread.uniqueIdentifier.workspaceId;
        }
        results.push(thread);
      });
    ...
    base.forEach((thread) => {
      ...
      if (
        thread.threadType === 'submitter' ||
        thread.threadType === 'approver'
      ) {

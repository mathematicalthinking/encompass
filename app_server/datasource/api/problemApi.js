...
async function getOrgRecommendedProblems(user, orgIds, isIdOnly = true) {
  try {
    let orgs = await models.Organization.find({ _id: { $in: orgIds } })
      .lean()
      .exec();
    if (!orgs) {
      return [];
    ...
    let filtered = _.filter(
      problems,
      (problem) => problem !== undefined && problem !== null
    );
  ...
  try {
    ...
    if (categories) {
      ...
      if (includeSubCats === 'true') {
        let catsWithChildren = await Promise.all(
          _.map(cats, (cat) => {
    ...
    if (pows) {
      ...
      if (org) {
        ...
        if (recommended) {
          recommendedProblems = await getOrgRecommendedProblems(
            user,
            recommended,
            true
          );
...
async function buildProblemsSearch(hash) {
  ...
  if (criterion === 'general') {
    ...
    let queries = [{ $text: { $search: query } }, searchFilter];
...
async function buildProblemsCriteria(user, queryParams, isTrashedOnly) {
  try {
    ...
    let [filter, search] = await Promise.all([
      buildProblemsFilter(user, filterBy),
      buildProblemsSearch(searchBy),
    ]);
...
const getProblems = async function (req, res, next) {
  ...
  try {
    ...
    const criteria = await buildProblemsCriteria(
      user,
      req.query,
      isTrashedOnly
    );
...
const postProblem = async function (req, res, next) {
  ...
  if (req.body.problem.privacySetting === 'E') {
    let isTitleUnique = await apiUtils.isRecordUniqueByStringProp(
      'Problem',
    ...
    if (!isTitleUnique) {
      return utils.sendError.ValidationError(
        `There is already an existing public problem titled "${req.body.problem.title}."`,
        'title',
        res
      );
    }
  ...
  try {
    ...
    if (req.body.problem.privacySetting === 'E') {
      let isTitleUnique = await apiUtils.isRecordUniqueByStringProp(
        ...
        req.body.problem.title,
        'title',
        { privacySetting: 'E', _id: { $ne: req.params.id } }
      );

...
var program = optimist
  .options({
    ...
    search: {
      describe:
        'Flag to process non-hyphenated options as part of find instead of update',
      alias: ['s'],
      boolean: true,
      default: false,
    },
  ...
  .usage(
    '\nCache accepts a source file or url path from which to retrieve JSON submission data and saves this to the database.\n' +
      ...
      'Usage: node cache [options] {file or url}'
  ).argv;
...
function formatAuditOptions() {
  ...
  var upCommands = ['inc', 'set', 'unset', 'push', 'addToSet', 'pull'];

  var allowMultiple = ['push', 'pull', 'addToSet'];

  if (!(_.isObject(query.find) && _.isObject(query.update))) {
    query.errors.push(
      new Error(
        'You must use dot notation to specify find and update criteria e.g find._id 123'
      )
    );
  }

  if (
    !_.any(upCommands, function (cmd) {
      return query.update.hasOwnProperty(cmd);
    })
  ) {
...
mongo.connect(options.url, function (err, db) {
  ...
  collection.update(
    options.find,
    options.update,
    { multi: options.multi, w: 1 },
    function (err, affected) {
      if (err) {
        console.log(err);
      }

      var note = util.format(
        noteFormat,
        new Date().toDateString(),
        options.collection,
        JSON.stringify(options.find),
        JSON.stringify(options.update),
        options.multi,
        affected
      );
      if (affected > 0) {

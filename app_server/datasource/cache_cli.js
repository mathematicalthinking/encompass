...
var program = optimist
  .options({
    ...
    id: {
      describe:
        'ID(s) of submissions to retrieve. Use multiple times for multiple IDs',
      type: 'number',
    },
    range: {
    ...
    date: {
      describe:
        'Specify the date to retrieve submissions from. Use twice to specify a date range',
      alias: ['d'],
      type: 'string',
...
function formatCacheOptions() {
  ...
  if (query._) {
    query.source = program._[0];
  }
  if (query.pd) {
    query.collection = program.pd;
  }
  delete query._;
  delete query.pd;

...
async function removeOrphanedFromWs(collection, workspaceIds) {
  ...
  console.log(
    `Deleted orphans count ${deleted} of ${allDocs.length} for ${collection}s`
  );
  console.log(`-------------------`);
  return deleted;
}
...
async function checkSubmissionWorkspaces() {
  ...
  try {
    ...
    console.log(
      `submission workspace array items deleted: ${deletedFromArray}`
    );
  } catch (err) {
    console.log(err);
  }
...
async function cleanSubmissionWorkspaces() {
  ...
  try {
    ...
    let workspaceIds = workspaces.map((ws) => ws._id);
    console.log(`workspaces.length: ${workspaceIds.length}`);
    let results = await models.Submission.updateMany(
      {},
  ...
  try {
    ...
    for (let id of colDocIds) {
      ...
      if (
        ...
        if (
          doc.lastModifiedDate === null ||
          doc.lastModifiedDate === undefined
        ) {
        ...
        console.log(
          `username: ${doc.username} - isAdmin: ${doc.isAdmin} - accountType: ${doc.accountType}`
        );

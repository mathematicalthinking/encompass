...
async function convertEditors() {
  try {
    ...
    console.log(
      `There are ${workspaces.length} workspaces with non empty permissions`
    );
    ...
    console.log(
      `There are ${folderValueis4Count} collabs with folder setting 4 to update`
    );
...
return convertEditors().then((results) => {
  console.log('done!');

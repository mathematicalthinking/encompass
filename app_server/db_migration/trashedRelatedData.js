...
function doesPopulatedWorkspaceHaveBadData(ws) {
  ...
  return (
    hasBadData(submissions) ||
    hasBadData(selections) ||
    hasBadData(comments) ||
    hasBadData(responses) ||
    hasBadData(folders) ||
    hasBadData(taggings)
  );
}
function doesPopulatedSubmissionHaveBadData(sub) {
  ...
  return (
    hasBadData(selections) || hasBadData(comments) || hasBadData(responses)
  );
}
...
function findSubmissions() {
  ...
  return models.Submission.find({})
    .populate('selections comments responses')
    .lean()
    .then((submissions) => {
      ...
      return submissions.filter((submission) => {
        ...
        if (numProcessed % 10000 === 0) {
          console.log(
            `Processed ${numProcessed} / ${submissions.length} submissions`
          );
        }
    ...
    .then((submissionsWithBadData) => {
      console.log(
        `There are ${submissionsWithBadData.length} submissions with bad data`
      );
    ...
    .then((detailObjects) => {
      ...
      let withBadSelections = detailObjects.filter(
        (obj) => obj.hasBadSelections
      );
      return {
...
function findWorkspaces() {
  ...
  return models.Workspace.find({})
    .populate('submissions selections responses comments folders taggings')
    .lean()
    .then((workspaces) => {
    ...
    .then((workspacesWithBadData) => {
      console.log(
        `There are ${workspacesWithBadData.length} workspaces who have bad Data`
      );
    })
    .catch((err) => {
      throw err;
...
function findSelections() {
  ...
  return models.Selection.find({})
    .populate('taggings comments')
    .lean()
    .then((selections) => {
    ...
    .then((selectionsWithBadData) => {
      console.log(
        `There are ${selectionsWithBadData.length} selections with bad data`
      );
    })
    .catch((err) => {
      throw err;
...
function findResponses() {
  ...
  return models.Response.find({})
    ...
    .then((responsesWithBadData) => {
      console.log(
        `There are ${
          responsesWithBadData.length
        } responses with bad data: ${responsesWithBadData.map((r) => r._id)}`
      );
    })
    .catch((err) => {
      throw err;
...
function findFolders() {
  ...
  return models.Folder.find({})
    .populate('chilren taggings')
    .lean()
    .then((folders) => {
    ...
    .then((foldersWithBadData) => {
      console.log(
        `There are ${foldersWithBadData.length} folders with bad data`
      );
    })
    .catch((err) => {
      throw err;
...
function findComments() {
  ...
  return models.Comment.find({})
    ...
    .then((commentsWithBadData) => {
      console.log(
        `There are ${commentsWithBadData.length} comments with bad data`
      );
    })
    .catch((err) => {
      throw err;

  ...
  try {
    ...
    let saved = workspaces.map(async (ws) => {
      ...
      if (isNonEmptyArray(missingWorkspaceTaggingIds)) {
        await models.Tagging.updateMany(
          { _id: { $in: missingWorkspaceTaggingIds } },
          { $set: { workspace: ws._id } }
        ).exec();
      }

      if (isNonEmptyArray(newTaggingIds)) {
        return models.Workspace.update(
          { _id: ws._id },
          { $addToSet: { taggings: { $each: newTaggingIds } } }
        ).exec();
      }
    });
...
function migrate() {
  return findMissingTaggings().then((res) => {

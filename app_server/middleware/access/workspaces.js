...
const canLoadWorkspace = function (user, ws) {
  ...
  if (accountType === 'S' || actingRole === 'student') {
    ...
    if (isCollaborator) {
      ...
      if (
        globalPermissions === 'directMentor' ||
        globalPermissions === 'indirectMentor' ||
        globalPermissions === 'approver'
      ) {
        return [true, null];
      }
  ...
  let isOrgWs =
    isOrgPrivacy && areObjectIdsEqual(user.organization, ws.organization);
  ...
  let isInPdOrg =
    areObjectIdsEqual(ownerOrg, userOrg) ||
    areObjectIdsEqual(creatorOrg, userOrg);
...
const accessibleWorkspacesQuery = async function (
  user,
  ids,
  filterBy,
  searchBy,
...
function canUpdateSubmissions(user, ws, updateType) {
  ...
  if (isPdAdmin && actingRole !== 'student') {
    ...
    if (
      areObjectIdsEqual(pdOrg, ownerOrg) ||
      areObjectIdsEqual(pdOrg, creatorOrg)
    ) {
      return true;
    }
  }
  ...
  let sectionTeachers = _.propertyOf(ws)([
    'linkedAssignment',
    'section',
    'teachers',
  ]);
  ...
  if (Array.isArray(sectionTeachers)) {
    isTeacher =
      _.find(sectionTeachers, (teacher) => {
  ...
  if (updateType === 'add') {
    ...
    if (Array.isArray(assignmentStudents)) {
      isStudent =
        _.find(assignmentStudents, (student) => {
